...
image:model-transformations.svg[float=right,width=400] To provide a preview of what is to come, an mm-ADT *model* defines an `obj` graph for a particular https://en.wikipedia.org/wiki/Domain_of_discourse[domain of discourse]. A transition from model \$A\$ to model \$B\$ occurs by way of the functor \[
