...
NOTE: image:model-transformations.svg[float=right,width=375] To provide a preview of what is to come, an mm-ADT *model* defines an `obj` graph for a particular https://en.wikipedia.org/wiki/Domain_of_discourse[domain of discourse]. A transition from model \$A\$ to model \$B\$ occurs by way of the functor \[
\texttt{Hom}(A,B) = \texttt{Hom}_A(\texttt{obj}_A,\texttt{obj}_A) \to \texttt{Hom}_B(\texttt{obj}\_B,\texttt{obj}_B)
\] known as a https://en.wikipedia.org/wiki/Natural_transformation[natural transformation] (or 2-path in https://en.wikipedia.org/wiki/Homotopy_type_theory[homotopy type theory]). Such model mappings allow types written in one https://en.wikipedia.org/wiki/Universe_(mathematics)[universe] to be evaluated within another universe, where, ultimately, all types must be grounded in the base `mm` model. It is through `mm` that the mm-ADT VM communicates with storage systems and processing engines, enabling arbitrary models atop a natural evaluation.
