...
.Constructive Type System
================================================================
Classical type theory correlates *types* and *sets*, where the type \$A\$ denotes a set containing all the elements of the type. To determine whether object \$x\$ is of type \$A\$, it is only necessary to check whether the set corresponding to the type contains \$x\$ via a membership function \$x \in A\$.

mm-ADT types are not sets, they are functions that https://en.wikipedia.org/wiki/Intuitionistic_type_theory[construct] objects of a given type (*range*) from objects of a given type (*domain*). The determination of whether \$a\$ is typed \$A\$ is whether the function \$A(a)\$ is defined at \$a\$.
...
| Declaring         | \$A\$ Type      | `A`
| Type assignment   | \$x:A\$         | `A:a` or `a\=>A`
| Object equality   | \$a = b\$       | `(a)\<=(b)`
| Type equality     | \$A = B\$       | `B\<=A`
| Type dependency   | \$(x:A)B\$      | `B\<=A`
| Type substitution | \$B[x//a]\$     | `a\=>B`
| \$\Sigma\$ type   | \$A \times B\$  | `(A;B)`
| \$\Pi\$ type   | \$A \to B\$        | `B\<=A`
|====

Note that type equality, dependency and \$Pi\$ all have the same form in mm-ADT. However, while type equality is a type dependency and a \$\Pi\$ type, every type dependency and \$\Pi\$ type is not a type equality.

The structure of an mm-ADT type maintains both the type's signature (`B\<=A`) and the type's definition `[f]`). The form of a typical type dependency is `B\<=A[f]`, where a `B` is _constructed_ from an `A` by applying instruction `[f]`. In standard mathematical notation, `B\<=A[f]` is denoted \$f: A \to B\$ which is the named version of the \$Pi\$ type \$A \to B\$ and therefore, a type dependency and a \$\Pi\$ type are the same construct in mm-ADT.

Finally, the type `B\<=A[f]` is not a type equality. The type equality \$A=B\$ is an https://en.wikipedia.org/wiki/Identity_function[identity function] `B\<=A[noop]`, where a `B` is realized by doing nothing to an `A`. The type `B\<=A[noop]` is more succinctly written `B\<=A` and thus, `B` is not _constructed_ from `A`, but is _equivalent_ to `A` and every `A` can be typed `B` as is.
================================================================
