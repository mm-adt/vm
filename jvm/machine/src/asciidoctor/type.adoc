...
The diagram above captures a fundamental structure in mm-ADT called the *obj graph*. The `obj graph` is used for https://en.wikipedia.org/wiki/Type_system#Type_checking[type checking], https://en.wikipedia.org/wiki/Type_inference[type inference], https://en.wikipedia.org/wiki/Optimizing_compiler[compiler optimization], https://en.wikipedia.org/wiki/Garbage_collection_(computer_science)[garbage collection], and more. The subgraph concerned with type definitions is called the *type graph*. The subgraph considered with mutation history (https://en.wikipedia.org/wiki/State_(computer_science)[state]) is called the *value graph*. The `obj` graph is the codomain of the link:https://en.wikipedia.org/wiki/Embedding[embedded] image of an `obj` ringoid called the *stream ring*. Both the `obj` graph and stream ringoid form the primary topics of study throughout the documentation.
