...
* **\$\Sigma\$ Types**: Contrary to their presentation in the table above, \$Sigma\$ types are more general than simple https://en.wikipedia.org/wiki/Product_(category_theory)[products]. \$Sigma\$ types are _dependent products_. As an example, the "ascending pair" \$\Sigma\$ type
\[
\texttt{apair} = \sum_{m:\mathbb{Z}} \sum_{n:\mathbb{Z}} m<n
\]
is a product of two integers \$m,n \in \mathbb{Z}\$, where \$m\$ is smaller than \$n\$ (\$m < n\$). This definitation specifies a relation between the components of the product (i.e., a dependence) that must be satisfied by every object of this type. In mm-ADT, \$\tt{apair}\$ is written `(int;int)<x>[is,<x>.0<x.1]`

[exec]
----
:[model,mm] %
 [define,apair<=(int;int)[is,<x>.0<x.1]]
(1;2)   => apair                          ## apair:(1;2)
