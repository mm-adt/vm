...
\[
(\texttt{obj},[;],[,],\;\underline{\mathbf{1}}\;\underline{\mathbf{0}}),
\]
where `obj` is the carrier set, `[;]` is the multiplicative binary operator, `[,]` is the additive binary operator, \$\underline{\mathbf{1}}\$ the multiplicative identity, and \$\underline{\mathbf{0}}\$ the additive identity. Along with the standard ring axioms, the `obj` stream ring respects the seven axioms of *stream ring theory*. mm-ADT defines three homomorphisms from the foundational `obj` stream ring algebra to component-specific algebras that inherit aspects of its structure.
...
4.+^h| Ring with Unity -- \$(\tt{obj},[;],[,],\underline{\mathbf{1}},\underline{\mathbf{0}})\$
...
4.+^h| Useful Deductions -- \$(\tt{obj},[;],[,],\underline{\mathbf{1}},\underline{\mathbf{0}})\$
...
Stream ring theory studies _quantified objects_. The quantifiers must be elements of an https://en.wikipedia.org/wiki/Ordered_ring[ordered ring] with unity. The most common quantifier ring is the integers with standard addition and multiplication, \$(\mathbb{Z},+,\ast,0,1)\$. However, the theory holds as long as the quantifiers respect the ring axioms and, when coupled to an object, they respect the stream ring axioms. The stream ring axioms are primarily concerned with quantifier mathematics and their significance in efficient stream computing.
...
The *bulking*, *merging*, and *removing* axioms are aimed at reducing the amount of data flowing through a stream. The *applying* and *splitting* axioms ensure quantifier semantics during \$\ast\$ and \$+\$ operations on the _object_ https://en.wikipedia.org/wiki/Semiring[semiring] are maintained, respectively. Without quantifiers and the stream ring axioms, the stream
\[
\{a,a,a,b,a,b,b,a,a\}
\]
...
.The Instruction Only Free Monoid
================================================================
...
The `[noop]` instruction is the only mm-ADT instruction that does not alter the state of the `obj` graph and thus, the state of the computation. In other words, there is no \$x \to_{\tt{[no\op]}} y\$ edge.
