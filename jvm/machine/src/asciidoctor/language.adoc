...
The program the mm-ADT VM, there must exist a functor from the source language to the logical structure of types and values within the mm-ADT.
...
Nested types are examples of _inter-instruction branching_. The more general form is _intra-instruction branching_. Any instruction that forks an `obj` into a `poly` or joins the `objs` of a `poly` into a stream are *branch instructions*. They generate *branching type traces*.

A https://en.wikipedia.org/wiki/Tagged_union[*sum type*] (tagged union) can be constructed from a ctype.
...
mm-ADT's type system is founded on the mathematics of https://en.wikipedia.org/wiki/Ring_(mathematics)[rings] with https://en.wikipedia.org/wiki/Unit_(ring_theory)[unity] -- the mm-ADT *type ring*. A ring relates two monoids that share a carrier set. The generator of the mm-ADT type ring's carrier set is the set of all instructions -- `inst`. The multiplicative (`*`) and additive (`+`) monoids of the type ring induce the set of all mm-ADT types. Ring theory specifies the following axioms, where the associated table provides the (most well known) logical entailments of said axioms -- i.e., theorems.
...
The ring axioms are presented again in the table below along with the associated `mmlang` expressions, where `[a]`, `[b]`, `[c]` are variable instructions from `inst`.
...
|`0 + a = a + 0 = a`                |`-<[[id]{0},[a]]>-` \$=\$ `-<[[a],[id]{0}]>-`
|`a - a = a + (-a) = 0`             |`-<[[a],[a]{-1}]>-` \$=\$ `[id]{0}`
|`a + b = b + a`                    |`-<[[a],[b]]>-` \$=\$ `-<[[b],[a]]>-`
|`(a * b) * c = a * (b * c)`        |`[map,[a][b]][c]` \$=\$ `[a][map,[b][c]]`
|`1 * a = a * 1 = a`                |`[id][a]` \$=\$ `[a][id]` \$=\$ `[a]`
|`(a + b) * c = (a * c) + (b * c)`  |`-<[[a],[b]]>-[c]` \$=\$ `-<[[a][c],[b][c]]>-`
|`a * (b + c) = (a * b) + (a * c)`  |`[a]-<[[b],[c]]>-` \$=\$ `-<[[a][b],[a][c]]>-`
|===

Example mm-ADT types demonstrating the equalities of the ring axioms are presented below in `mmlang`, where `[a]` is `int[plus,1]`, `[b]` is `int[mult,2]`, and `[c]` is `int[mult,3]` (though any respectfully sorted types can be used).

.mmlang Expressions of the Ring Axioms
[cols="5,5"]
|===
a|
[source]
...
==>2
mmlang> 1-<[[plus,1],[plus,1]{-1}]>-
mmlang> 1[id]{0}
mmlang> 1-<[[plus,1],[mult,2]]>-
==>2{2}
mmlang> 1-<[[mult,2],[plus,1]]>-
==>2{2}
mmlang> 1[map,[plus,1][mult,2]][mult,3]
==>12
mmlang> 1[plus,1][map,[mult,2][mult,3]]
==>12
----
