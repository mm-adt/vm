...
Every mm-ADT program can be interpreted as a single https://en.wikipedia.org/wiki/Unary_function[unary function] from an `obj` of type \$S\$ (start) to an `obj` of type \$E\$ (end), where
\[
f: S \rightarrow E.
\]

An `obj` is a *quantified* object that is either a *type* or a *value*, where
\[
...
Types denote _streams_ (i.e. https://en.wikipedia.org/wiki/Multiset[multi-set] or bag) of values. Values denote datum with a structure pre-defined outside the mm-ADT VM within the VM's larger execution context. For instance, if an mm-ADT VM is implemented on the https://en.wikipedia.org/wiki/Java_virtual_machine[JVM], then the `str` value `'mmadt'` refers to the `java.lang.String` instance `"mmadt"`. Both `obj` types and values have associated quantifiers (`q`). The meaning of quantification is dependent on the quantifier chosen (e.g. integers, reals, unitary matrices, etc.). However, in general, the intuition is that the quantifier denotes the _amount_ of values being referred to. A quantified type states how many values the type refers to. A quantifier value states how many identical values are being denoted by the single value encoding.

=== Types

Types are derived from a set of base types via laws of composition that respect the axioms of https://en.wikipedia.org/wiki/Ring_theory[algebraic ring theory]. Type compositions ultimately specify programs, streams, functions, and of particular import, https://en.wikipedia.org/wiki/Functor[functorial] mappings between different https://en.wikipedia.org/wiki/Abstract_data_type[abstract data types].
...
image::type-monoid.png[float="left",width=40]

mm-ADT types serve numerous roles which are typically realized by many different sorts of objects in traditional programmable systems. The reason for this singular use is quite literally because the mm-ADT type system is induced from a generating set of types in the https://en.wikipedia.org/wiki/Monoid[monoid]
\[
\langle\texttt{type},\ast\rangle,
\]
where \$\tt{type}\$ is the carrier set is the set, \$\ast: \tt{type} \times \tt{type} \rightarrow \tt{type}\$ is the binary operator, and where the subset \$\tt{type} \subset (\tt{ctype} \cup \tt{i\nst})\$ (definitions forthcoming) is the seed set that generates the syntactic structure of all possible mm-ADT types and thus, programs. The example belows demonstrates the type-monoid where \$\ast\$ is juxtaposition or element concatenation.
...
image::obj-full.png[float=left,width=300]
