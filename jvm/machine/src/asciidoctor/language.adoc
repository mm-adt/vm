...
A `|-poly` (pronounced "pipe poly") uses `|` as its `obj` term separator. Like the `,-poly`, a `|-poly` maintains a free additive group and a non-free multiplicative monoid. However, the additive group is not commutative. If \$a,b,x \in \tt{objects}\$ and \$q_0,q_1,q_2 \in \tt{q}\$, then `|-poly` \$[a_{q_0} | b_{q_1}]\$ acts on \$x_{q_2}\$ as
...
While `,-lst` implements link:https://en.wikipedia.org/wiki/Union_(set_theory)[union], `|-lst` implements a https://en.wikipedia.org/wiki/Null_coalescing_operator[null coalescing operator], where in mm-ADT, "`null`" is `obj{0}` (the zero element of the `obj` stream ring). The reason every `|-poly` is not commutative is because the order in which the terms/branches are evaluated determines the result of the computation.
...
`|-rec` extends `|-lst` with https://en.wikipedia.org/wiki/Conditional_(computer_programming)[conditional branching], where branch selection is not based on \$\Bool\$ (`bool`), but on \$\tt{q}\$ (`{q}`), where "`false`" is `obj{0}` and "'true'" is any non-zero quantifier. If \$a,b,c,d,x \in \tt{objects}\$ and \$q_0,q_1,q_2,q_3,q_4 \in \tt{q}\$, then

\[
[a_{q_0} -> b_{q_1}  | c_{q_2} -> d_{q_3}](x_{q_4}) =
\begin{cases}
