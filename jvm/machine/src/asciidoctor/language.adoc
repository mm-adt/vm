...
|AST         | type       | a single https://en.wikipedia.org/wiki/Intermediate_representation[intermediate representation] is used in compilation, optimization, and evaluation.
|state       | trace      | types and values both encode state information in their process traces.
|functions   | types      | functions are (dependent) types with values generated at evaluation.
