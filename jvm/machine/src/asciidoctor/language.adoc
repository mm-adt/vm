...
Every `obj` exists as a distinct vertex in the `obj` graph. If \$b \in \tt{obj}\$ has an incoming edge labeled \$i \in \tt{i\nst}\$, then when applied to the outgoing adjacent vertex \$a\$, \$b\$ is computed. Thus, the `obj` graph edge \$a \to_i b\$ provides information as the history of the computation. These edges can be followed back to root such that the result composition is a *path*.

[exec]
----
8 => int[plus,1][mult,2][gt,10]
8 => int[plus,1][mult,2][gt,10][trace]
----
