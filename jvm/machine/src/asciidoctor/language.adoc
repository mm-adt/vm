...
The `obj` structure satisfies the axioms of *stream ring theory*. The algebraic structure is
\[
(\texttt{obj}\;;\;,\;\mathbf{1}\;\mathbf{0}).
\]

The following table provides a review of the ring theoretic axioms and their realization in mm-ADT via `mmlang` over both `obj` *values* and *types*. This table serves as a consolidated summary and the particulars surrounding their meaning and use forms the primary purpose of this documentation.

[.small]
[cols="1,1,3,6,7"]
|===
| structure |  axiom | equation  | mmlang values | mmlang types
