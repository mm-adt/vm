...
The mm-ADT virtual machine is programmed by composing objects (`obj`) using instructions (`inst`) parameterized by objects. The mm-ADT VM is written in https://scala-lang.org/[Scala]. The core library exposes a fluent program builder https://en.wikipedia.org/wiki/Application_programming_interface[API] called `mmscala`. Most compiler designers should avoid `mmscala` due to the complexities that may ensue given direct access to the https://en.wikipedia.org/wiki/Java_virtual_machine[JVM]. Instead, `mmlang` should be the target language for most compilers.
