...
[.small]
[cols="1,3,1,1",width=35%,float=right]
|===
| type | inst                              | 0       |  1
...
The mm-ADT atomic, non-composite types are denoted `mono`. There are 4 `mono` types: `bool`, `int`, `real`, and `str`. The associated table presents the typical operators (sugar'd instructions) that can be applied to each type as well as the identity (additive) and annihilator (multiplicative) values of the type. This section will present example uses of each type.
...
image:bool-zero-one.png[width=265,float=left] When an `obj` is applied to an `inst`, the `inst` yields an `obj`. In mm-ADT, types and values are simply vertices in the `obj` graph. A https://en.wikipedia.org/wiki/Polymorphism_(computer_science)[polymorphic] https://en.wikipedia.org/wiki/Constant_function[constant] `inst` such as `[zero]` and `[one]` require only the domain's type to produce a respective constant value. Consequently, the distinction between compilation and evaluation blur. That is, "compilation" can produce values and thus, the program is complete before being "evaluated." This idea is exaggerated in the example below, where from the `bool` ctype, instructions are applied one-by-one to demonstrate how the type's definition (a path in the Cayley `obj` graph) yields a `false` value. At `[zero]`, the process transitions from the type graph to the value graph which are both connected subgraphs of the larger `obj` graph. The final instruction, `[and,bool,bool]`, is compiled to `[and,false,false]` which results in `false`. Thus, a single "compilation" automata is able to derive the result set.

[exec]
----
bool
bool[or,bool,bool]
bool[or,bool,bool][zero]
