...
This internal structure is well-defined as an https://en.wikipedia.org/wiki/Ring_%28mathematics%29[algebraic ring].
...
Many modern programming environments make a sharp distinction between types and values, where each is prominently used for either https://en.wikipedia.org/wiki/Compiler[compilation] (types) or evaluation (values).
In mm-ADT, types and values co-exist and are both computable entities.
For instance, types that exist at evaluation-time are what are generally known as https://en.wikipedia.org/wiki/Anonymous_function[lambda (or anonymous) functions].
...
This is not the case. mm-ADT types are a coproduct where every type is either a
...
In terms of the "__range = domain and function__" reading, when `int` (*domain*) is applied to `[is>0]` (*function*), the result is an integer greater than zero (*range*).
...
Both the domain and range type specifications include a respective quantification denoted `{q}` in `mmlang`.
...
When the instruction `[plus,4]` is applied to the mm-ADT `int` value `2`, a new mm-ADT `int` value is created whose ground is `6L`.
...
However, in practice, the mm-ADT VM removes a trace (a particular path through the graph) when the current mm-ADT program will no longer require the information contained therein.
