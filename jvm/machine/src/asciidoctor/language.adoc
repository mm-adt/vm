...
. *Data structures* (_values_): A value-based `poly` is realized as a _determinate polynomial_ (i.e., a polynomial lacking indeterminates). mm-ADT provides two convenient `poly` type constructors for working with `poly` data structures: `lst` (an `int`-indexed array) and `rec` (an `obj`-indexed array).
...
\]
where the _branch_ component is, in fact, a group with `-<[a{q},a{-q}]>- = obj{0}` and `[branch,a{q},a{-q}] = obj{0}`. The `|-poly` can be appended to the `;,-poly` ring, where it serves as an https://en.wikipedia.org/wiki/Idempotence[idempotent] variation of the additive group commonly used to denote https://en.wikipedia.org/wiki/Tagged_union[sum types].

.Delta Poly
[cols="4,1,8",width=45%,float=left]
|===
...
.Nabla Poly
[cols="4,1,8",width=45%,float=right]
|===
| poly op            | mmlang      | example
