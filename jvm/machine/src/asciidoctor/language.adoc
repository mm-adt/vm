...
mm-ADT represents computations via https://en.wikipedia.org/wiki/Unary_function[unary functions]. For instance, the instruction `[neg]` takes an input `int` and returns the inverse (or negative) `int`. Thus, `1 \=> int[neg]` outputs -1. The `[neg]` instruction acts as a classic unary function. Next, increasing the complexity, the instruction `[plus,2]` is unary even though it has a constant argument of `2`. Thus, if the input is 6, ultimately the https://en.wikipedia.org/wiki/Binary_function[binary function] \$6+2\$ is evaluated. Regardless, the argument value is static as `[plus,2]` acts in a manner analogous to a https://en.wikipedia.org/wiki/Currying[curried] unary function.
...
\]
|===
If the argument to `[plus]` is a type (as opposed to a value), then the argument's value resolution is dependent on the incoming `obj`. For instance, if `int[plus,[mult,2]]` has an incoming `int` of 6, the argument to `[plus]` is 12. However, if the incoming `int` is 8, then the `[plus]` argument is 16. Thus, while mm-ADT can be understood as the serial and parallel composition of unary functions, function behavior, via types, can be made https://en.wikipedia.org/wiki/Dependent_type[dependent] on the state of the incoming `obj`.

====== Path Dependent

Every `obj` exists as a distinct vertex in the `obj` graph. If \$b \in \tt{obj}\$ has an incoming edge labeled \$i \in \tt{i\nst}\$, then when applied to the outgoing adjacent vertex \$a\$, \$b\$ is computed. Thus, the `obj` graph edge \$a \to_i b\$ provides information as the history of the computation. These edges can be followed back to root such that the result composition is a *path*. The path is accessed via `[trace]` which is returned as a `;-lst`. This `lst` can then be introspected as any other `lst`.

[exec]
----
8 => int[plus,1][mult,2][gt,10]                        //<1>
8 => int[plus,1][mult,2][gt,10][trace]                 //<2>
8 => int[plus,1][mult,2][gt,10][trace][get,5][get,0]   //<3>
----
<1> The evaluation of an `bool\<=int` type via 8.
<2> The `obj` graph path from 8 to `[gt,10]`.
<3> A projection of the instruction `[gt,10]` from the path and then the first argument of the `inst`.

mm-ADT has a collection of *distinct identity instructions*, such that as instructions, they simply compute \$f(x) \mapsto x\$, but as edge labels in the `obj` Cayley graph, they store computational state that can be accessed in the future via path analysis. In effect, the execution context is transformed from a memory-less https://en.wikipedia.org/wiki/Finite-state_machine[finite state automata] into https://en.wikipedia.org/wiki/Processor_register[register]-based https://en.wikipedia.org/wiki/Turing_machine[Turing machine].
