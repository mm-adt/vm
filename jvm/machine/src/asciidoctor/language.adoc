...
There are three kinds of `polys`: `vpoly`, `tpoly`, and `tvpoly`. The `vpolys` and `tpolys` are fixpoint structures denoting purely data or process structures and exist at the extreme ends of the type/value `obj` divide. Between these extremes are the `tvpolys`. These `polys` can be understood as representing either a process that hasn't returned terminated (evaluation) or a data structure containing functions (references).
...
. *Computational structures* (_computing_): a type/value `poly` is a `tvpoly` (i.e., a `poly` at an intermediate state between a `tpoly` or `vpoly` fixpoint).
...
When the `tvpoly` is solved using `-<` and `>-`, the determinate form is realized as a `vpoly` (the fixpoint).
