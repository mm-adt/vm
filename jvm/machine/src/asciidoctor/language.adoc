...
The full `obj` structure thus far is diagrammed on the right. On the left, some `mmlang` examples are presented.
...
The salient features of a type are captured in the pseudo-`mmlang` expression below, where the three projections encode a type's
...
image::type-value-trace.png[align=center,width=650]
...
The mm-ADT VM compiler translates an `mmlang` expression to a subgraph of the larger Cayley type graph. The compiler then walks that subgraph from root to leaves filling in any information that was not specified in the original expression. This technique is made salient in the compiler field of https://en.wikipedia.org/wiki/Abstract_interpretation[abstract interpretation]. Amongst other benefits, it allows users to write expressions containing few types and quantifiers as they can be deduced by the type graph structure. This is called https://en.wikipedia.org/wiki/Type_inference[type inference]. The expressions above are rewritten below in order to demonstrate mm-ADT inferencing.
...
If the `objs` of the multiplicative monoid are values (as opposed to free types), then the expression above would further reduces to

[.text-center]
`(x0{q0},x1{q1},x2{q2},x3{q3})`.

In general, an mm-ADT `poly` is used to _decouple_ one algebraic structure from another in order that one be free and the other non-free. From the perspective of applied computing, the free algebras are https://en.wikipedia.org/wiki/Lazy_evaluation[delayed], unevaluated structures and the unfree algebras are https://en.wikipedia.org/wiki/Eager_evaluation[eager], evaluated structures. Each type of `poly` realizes a particular algebraic structure with useful computing properties.

. https://en.wikipedia.org/wiki/Polynomial_ring[polynomial rings] (`,-poly`): nondeterministic parallel branching and products (_orThen_).
. https://en.wikipedia.org/wiki/Monoid_ring[monoid rings] (`|-poly`): deterministic `if/else`-branching and co-products (_xorThen_).
. https://en.wikipedia.org/wiki/Trace_monoid[trace monoids] (`;-poly`): type rewriting and meta-programming (_andThen_).


'''

image::poly-type-value.png[float=right,width=500]

In mm-ADT, `poly` is formed from the coproduct of `lst` and `rec`.

mm-ADT provides two convenient type constructors for working with `poly` data structures: `lst` (an `int`-indexed array or https://en.wikipedia.org/wiki/List_(abstract_data_type)[list]) and `rec` (an `obj`-indexed array or https://en.wikipedia.org/wiki/Record_%28computer_science%29[record]). Both `lst` and `rec` sets are composed of value and type forms.
...
A https://en.wikipedia.org/wiki/Cayley_graph[Cayley graph] is a graphical encoding of the structure of a group. If \$\langle A, \cdot, I \rangle \$ is a group with carrier set \$A\$, binary operator \$\cdot : (A \times A) \to A\$, and generating set \$I \subseteq A\$ then the graph \$G = (V,E)\$ with vertices \$V = G\$ and edges \$E = (A \times (A \times I))\$ is the Cayley graph of the group. Thus, the edge \$a \to_i b\$ states that the outgoing and incoming vertices \$a,b \in A\$ respectively are connected by an edge labeled with the element \$i \in I\$.

A _fully constructed_ graph, a Cayley graph's vertices are the group elements and its edges represent the set of possible transitions from any one element to the next given the generators. As a _dynamically constructed_ graph, a Cayley graph encodes the history of a group computation, where the current element has an incoming edges from the previous element being applied to a denoted generator. A _generalized_ Cayley Graph applies the same concepts of the Cayley graph's group presentation, to other magmas such as monoids and semigroups.
****
