...
The ctypes are fundamental, atomic types. Everything else, regardless of being an _explicit_ `obj` subject to instruction manipulation or an _implicit_ object within the mm-ADT VM's host environment, is a `poly` (https://en.wikipedia.org/wiki/Polynomial[polynomial]). mm-ADT `polys` encode `obj` compositions in a non-reductive manner as a https://en.wikipedia.org/wiki/Trace_monoid[trace monoid], where the specified composition operator relating the `poly` components is either a _parallel_ (`,`), _serial_ (`;`) , and _choice_ (`|`). Each operator determines the denotational semantics of the endo/poly-morphic instructions `[mult]`/`[div]` and `[plus]`/`[minus]`. The `poly` serves two primary roles in mm-ADT:

. *Composite structures* (_values_): A value-based `poly` is not reduced by the associated monoid operator, but instead realizes as a _determinate polynomial_ (a polynomial lacking indeterminates). mm-ADT provides two convenient value-based `poly` type constructors: `lst` (an `int`-indexed array) and `rec` (an `obj`-indexed array).
. *Branching processes* (_types_): A `poly` can be reduced using the `[split]`/`[merge]` "delta-nabla"-instructions, where `[split]` splits the incoming `obj` across the `poly` components, where if the component is a type, the `obj` is resolved to an element of the type's range. In this manner, a type-based `poly` is a \$n\$-ary branch structure, where the trace operator determines the branch semantics.

NOTE: An algebraic representation of the mm-ADT VM's underlying calculus is realized by a "lifting" https://en.wikipedia.org/wiki/Functor[functor] that maps objects and morphisms of the virtual machine's host context to the category of mm-ADT `polys`. Given that a `poly`, like any other `obj`, can be operated on by virtual machine instructions, `polys` are the gateway to mm-ADT https://en.wikipedia.org/wiki/Metaprogramming[metaprogramming].

====== Composite Structures

====== Branching Processes

A `poly` type is https://en.wikipedia.org/wiki/Type_constructor[constructed] and destructed using \$\Delta\$ (_delta_) and \$\nabla\$ (_nabla_), respectively. This section specifies the properties of the `polys` diagrammed below, where `,-poly` is a faithful implementation of the type ringoid's additive group (\$oplus\$), `|-poly` is an implementation of an https://en.wikipedia.org/wiki/Idempotence[idempotent] variation of the additive group commonly used to denote https://en.wikipedia.org/wiki/Tagged_union[sum types], and `;-poly` is a faithful implementation of the type ringoid's multiplicative monoid (\$otimes\$).
