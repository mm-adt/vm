...
[ a,a,b,a,b,b,a,a ]
\]
is irreducible. However, with the stream ring axioms and \$\mathbb{Z}\$-quantifiers, the above stream is equivalent to
\[
[ 5a,3b ],
\]
...
An algebraic https://en.wikipedia.org/wiki/Ring_(mathematics)[ring] is composed of an additive abelian group \$(A,\+,\mathbf{0})\$ and a multiplicative monoid \$(A,\ast,\mathbf{1})\$ that share the same carrier set \$A\$. A https://ncatlab.org/nlab/show/Ab-enriched+category[ringoid] generalizes a ring to a multi-sorted carrier \$A = (A_0,A_1,\ldots,A_n)\$ such that the binary operators are https://en.wikipedia.org/wiki/Partial_function[partial functions] lacking https://en.wikipedia.org/wiki/Closure_(mathematics)[closure]. mm-ADT leverages this structure in the _process algebra_ called *the type ringoid*.
\[
(\texttt{,;-lst},[,],[;],\underline{\mathbf{0}},\underline{\mathbf{1}}),
\]
where

* `,;-lst`: types freely encoded as `lsts`.
* `[,]`: a free additive group,
* `[;]`: a free multiplicative monoid,
* `_{0}`: the additive identity, and
* `_{1}`: the multiplicative identity.

The type ringoid's https://en.wikipedia.org/wiki/Generator_(mathematics)[generator] is the set of all `[ctypes]` and single instruction `[ctype;inst]`, where a faithful application according to sort, induces the set of all possible free mm-ADT types.

[.small]
[exec]
----
int[plus,10][is,[gt,10]][lt,50]                                                              //<1>
[10,40] => int[plus,10][is,[gt,10]][lt,50]
-<(int;[plus,10];int;[is,-<(int;[gt,10];bool)>-];int{?};[lt,50];bool{?})>-                   //<2>
[10,40] => -<(int;[plus,10];int;[is,-<(int;[gt,10];bool)>-];int{?};[lt,50];bool{?})>-
----
...
\[
\begin{split}
  \nabla( a^1_{q_1}, a^2_{q_2}, \ldots, a^n_{q_n} ) &= [a^1_{q_1}, a^2_{q_2}, \ldots, a^n_{q_n}], \\
\nabla( a^1_{q_1}; a^2_{q_2}; \ldots; a^n_{q_n} ) &= a^1_{q_1} => a^2_{q_2} => \ldots => a^n_{q_n}
\end{split}
\]
