...
If every `obj` is either a quantified type or a quantified value, then types and values must have more structure than represented in the previous `obj` equation
\[
\texttt{obj} = (\texttt{type} \times \texttt{q}) + (\texttt{value} \times \texttt{q}).
\]

image::type-product.png[float=left,width=295]

If no other structure existed, then values and types would be isomorphic and thus, indistinguishable. mm-ADT has six nominal types and two structural types. The nominal types are the the base or fundamental types called *canonical types* (ctypes). The structural types are *derived types* ultimately grounded in one of the canonical types via chain of instructions (`inst`) that operate on a types to yield types. This is captured in the equation
\[
\begin{split}
...
\end{split}
\]
where each component structure has a respective `{q}`.
