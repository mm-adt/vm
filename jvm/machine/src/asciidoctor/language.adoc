...
The ctypes are fundamental, atomic types. Everything else is a `poly` (https://en.wikipedia.org/wiki/Polynomial[polynomials]) regardless of it being explicitly encoded as an `obj` subject to manipulation by the instruction set architecture or implicitly within the underlying machinery of the mm-ADT VM. mm-ADT `polys` encode `obj` compositions in a non-reductive manner as a https://en.wikipedia.org/wiki/Trace_monoid[trace monoid], where the composition operator relating the `poly` components are either _parallel_ (`,`), _serial_ (`;`) , and _choice_ (`|`). These operators specify the denotational semantics of the polymorphic, endomorphic instructions `[mult]`/`[div]` and `[plus]`/`[minus]`. The mm-ADT `poly` has numerous uses in mm-ADT with it's two primary roles being:

. *Composite structures* (_values_): A value-based `poly` is not reduced by the associated monoid operator, but instead is realized as a polynomial lacking indeterminates. mm-ADT provides two convenient value-based `poly` types: `lst` (`int`-indexed array) and `rec` (`obj`-indexed array).
. *Branching processes* (_types_): When an `obj` is passed as an argument to an additive `poly` group, the `obj` is split across the individual `poly` components, where if the component is a type, the `obj` is resolved to an element of the type's range whereby the component serves as branch in the computation.

NOTE: Given that a `poly`, like any other `obj`, can be operated on by the mm-ADT VM instructions and that they provide a "lifted" algebraic representation of the mm-ADT VM's underlying calculus, `polys` are the entry-point to mm-ADT https://en.wikipedia.org/wiki/Metaprogramming[metaprogramming].

====== Composite Structures

====== Branching Processes

A `poly` type is https://en.wikipedia.org/wiki/Type_constructor[constructed] and destructed using \$\Delta\$ (_delta_) and \$\nabla\$ (_nabla_), respectively. This section specifies the properties of the two additive `polys` diagrammed below, where `,-poly` is a faithful implementation of a ring `+` operator and `|-poly` is a faithful implementation of an https://en.wikipedia.org/wiki/Idempotence[idempotent] variation of the ring `+` operator commonly used for https://en.wikipedia.org/wiki/Tagged_union[sum types].

[cols="3,^9"]
|===
| `,-poly`
\[
  \left(\Delta^, \cdot \langle f,g \rangle \cdot \nabla^,\right)
\]
