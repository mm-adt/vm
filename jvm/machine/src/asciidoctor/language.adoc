...
There are two distinct kinds of mm-ADT `objs`: _quantified_ type objects and _quantified_ value objects. The products of the `obj` coproduct are called *type* and *value*. That is the *obj meta-model*.

\[
\texttt{obj} = \texttt{type} + \texttt{value}
\]

NOTE: There are only a few instances in which it is necessary to consider the object component of an `obj` separate from its quantifier component. The terms _type_ and _value_ will always refer to the object/quantifier-pair as a whole -- i.e., an `obj`.
...
A *type signature* specifies the source and target of this mapping, where the *domain* is the source type, and the *range* is the target type. As ctype `objs`, the domain and range each have an associated quantifier denoted `{q}` in `mmlang`.
The general pattern of a type signature is

[.center]
[width=30]
|===
^a|[exec,eval=false,exception=LanguageException]
----
range{q}<=domain{q}
----
{blank}
...
For a type, the connected chain of references is called the *type definition* and is a encoded as a https://en.wikipedia.org/wiki/Path_(graph_theory)[path] in the *type graph*. For a value, the *value graph* encodes an analogous path called the *value history* (or _mutation history_). Both types and values exist in a larger graph called an *obj graph*. The https://en.wikipedia.org/wiki/Commutative_diagram[commutative diagram] below denotes an `obj` graph composed of two paths. The top horizontal path is a value history and the bottom horizontal path is a type definition. These paths are joined by the only instruction in `inst` that unites the type graph and value graph: `[type]`.
...
image::type-value-path.svg[align=center,width=600]
...
In theory, the complete history of an mm-ADT program (from compilation to execution) is stored in this graph. However, in practice, the mm-ADT VM removes paths in the graph that are no longer required by the program. This process is called *path retraction* and is the mm-ADT equivalent of https://en.wikipedia.org/wiki/Garbage_collection_(computer_science)[garbage collection].
...
Depending on the context of use, each interpretation of the `obj` graph will be leveraged: from compilation to evaluation. For example, the object graph's commutativity is leveraged for both compile-time and runtime https://en.wikipedia.org/wiki/Graph_rewriting[type rewriting]. If two paths have the same source vertex (domain) and the same target vertex (range), then both paths yield the same result. In practice, the VM is biased towards evaluating the instructions along the https://en.wikipedia.org/wiki/Computational_complexity_theory[computationally cheaper] path. As a generalized, multi-rooted monoidal Cayley graph, the set of all possible mm-ADT computations exists theoretically given the https://en.wikipedia.org/wiki/Presentation_of_a_monoid[monoid presentation] containing the root `objs` (e.g. the ctypes) and its generators (`inst`). This static immutability is a perfect medium for https://en.wikipedia.org/wiki/Memoization[memoized] computations -- especially in data-intensive, cluster-oriented environments where storage is cheap and processors are costly.
================================================================
