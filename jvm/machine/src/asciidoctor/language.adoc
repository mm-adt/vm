...
| either   | `\|` | \$o+\$   | `[choose]`
|===
The mm-ADT `poly` (https://en.wikipedia.org/wiki/Polynomial[polynomial]) is both a _ctype_ and an element of either a _serial_ (`;`), _parallel_ (`,`), or _choice_ (`|`) https://en.wikipedia.org/wiki/Free_object[free] https://en.wikipedia.org/wiki/Trace_monoid[trace monoid] that is https://en.wikipedia.org/wiki/Adjoint_functors[left-adjoint] to one of the mm-ADT VM operational monoids -- \$langle \tt{obj},\ast \rangle\$, \$langle \tt{obj},+ \rangle\$, or \$langle \tt{obj},\o+ \rangle\$, respectively. A \$\nabla\$ (_nabla_) is an https://en.wikipedia.org/wiki/Epimorphism[epi] - https://en.wikipedia.org/wiki/Catamorphism[catamorphism] that folds the free `poly` structure into an `obj` by way of a reductive, "left-to-right" application of the `obj` monoid's operator. Both the `-poly` and \$\langle \tt{obj},- \rangle\$ monoids are derived from the set of mm-ADT `objs`, where the _delta morphisms_, \$\Delta:\tt{obj}^n \rightarrow \tt{poly}\$,  *construct* `polys` and the compositional `inst` instructions (`[map]`, `[branch]`, and `[choose]`) *compute* `objs`. The commuting diagrams below expose the https://en.wikipedia.org/wiki/Universal_property[universal property] of monoid mappings across the mm-ADT monoids.

image::monoid-homomorphisms.png[align=center]

Finally, the three monoids within each monoidal category compose into a universal algebraic https://zenodo.org/record/2565243[stream ring]. The `poly` monoidal construction realizes a https://en.wikipedia.org/wiki/Polynomial_ring[polynomial ring] and thus, a syntactic https://en.wikipedia.org/wiki/Free_algebra[free algebra] enabling mm-ADT https://en.wikipedia.org/wiki/Metaprogramming[metaprogramming] via the https://en.wikipedia.org/wiki/Ring_homomorphism[ring homomorphism] from the structural `;,-poly` ring to the procedural \$\langle \tt{obj},\ast,+\rangle\$ ring -- the mm-ADT virtual machine.
