...
image::obj-type-value-q.png[float=left,width=375]
...
image::obj-type-q-value-q.png[float=right,width=375]
...
\]
This representation, which is https://en.wikipedia.org/wiki/Isomorphism[isomorphic] to the previous, states two distinct kinds of mm-ADT `objs`: *types* and *values*.
...
Both types and values can be operated on by types, where each is predominately the focus of either https://en.wikipedia.org/wiki/Compiler[compilation] (types) or https://en.wikipedia.org/wiki/Execution_(computing)[evaluation] (values). Two notable consequences of computable types and values are
...
image::obj-full.png[float=right,width=275]
...
image::commuting-diagram.png[float=right,width=125]
...
independent terms +
\[A \times A = 2A\]
| image:comma-delta-nabla.png[width=475,link=images/language/comma-delta-nabla.png]
...
dependent terms +
\[A + A = A\]
| image:pipe-delta-nabla.png[width=475,link=images/language/pipe-delta-nabla.png]
|===
