...
\nabla^;(\texttt{poly}) &=& \texttt{[branch}, \texttt{poly}^{q_0}_0, \texttt{poly}^{q_1}_1,\ldots, \texttt{poly}^{q_n}_n,\texttt{]} \text{ with } q=(\min_{i<n}(q_i),\max_{i<n}(q_i)),
\end{split}
\]
where `\nabla` is realized as the https://ncatlab.org/nlab/show/codiagonal[codiagonal] `>-` (i.e. `[merge]`) instruction. the resultant `obj` is quantified within the specified `q` range. This is the general solution to deriving the the type quantifier during compilation. This range can be further refined with instruction semantics.

'''
image::free-monoid-homomorphisms.png[align=center]
'''

Both the `-poly` and \$\langle \tt{obj},- \rangle\$ monoids derive their carrier sets from the https://en.wikipedia.org/wiki/Initial_and_terminal_objects[initial] set of mm-ADT `objs`. The injective _delta functions_, \$\Delta:\tt{obj} \rightarrow \text{-poly}\$, yield the generators of the `-poly` monoid's, where
\[
\begin{split}
...
\end{split}
\]
and `\Delta` is realized as the https://en.wikipedia.org/wiki/Diagonal_morphism[diagonal] `-<` (i.e. `[split]`) instruction. Likewise, `[compose]`, `[branch]`, and `[choose]` are the instruction representations of the composition of `objs`. If
\[
\begin{split}
U: M \rightarrow \textbf{Set}
\end{split}
\]
maps a monoid to it's carrier set, then the diagrams below commute such that the https://en.wikipedia.org/wiki/Universal_property[universal property] of monoid mappings is realized in the lifted `poly` syntactic category.

image::monoid-homomorphisms.png[align=center]

Finally, the corresponding monoids across the three diagrams are the monoids of a particular encoding of the universal https://zenodo.org/record/2565243[stream ring] algebra. The `poly` monoids form a https://en.wikipedia.org/wiki/Polynomial_ring[polynomial ring] and thus, a syntactic https://en.wikipedia.org/wiki/Free_algebra[free algebra] that enables mm-ADT https://en.wikipedia.org/wiki/Metaprogramming[metaprogramming] by way of a https://en.wikipedia.org/wiki/Ring_homomorphism[ring homomorphism] from the *structural* ` free ring to the *procedural* reducing ring.
\[
;,-\texttt{poly}  \xrightarrow{\;\;\;\nabla^{;}\;\;\;} \langle \texttt{obj},\texttt{[compose][branch]}, \rangle
\]
where the _branch_ component forms a group with `-<[a{q},a{-q}]>- = obj{0}` and `[branch,a{q},a{-q}] = obj{0}`.
