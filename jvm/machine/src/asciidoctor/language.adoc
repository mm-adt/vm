...
. *terms*: `objs` connected by the multiplicative operator `;` (_andThen_).
.. *indeterminates*: type `objs` known as variables or symbols.
.. *constants*: value `objs` known as constants.
. *sum composites*: the symbols `,` (_orThen_) and `|` (_xorThen_) relate terms by a function with the general form

\[
\texttt{obj} = \sum_{i<n} (v_i \cdot t_i).
\]

If \$V \subset \mathbf{Obj} \$ are the set of all mm-ADT `obj` values, \$T \subset \mathbf{Obj}\$ the set of `obj` types, \$V \cap \T = \emptyset \$, then the polynomial \$ \tt{poly} \in \tt{Poly} \$ is denoted in conventional algebraic notation as one of the following expressions
\[
\begin{split}
(v_0 \cdot t_0) & + & (v_1 \cdot t_1) & + & \ldots & + & (v_n \cdot t_n) \\
(v_0 \cdot t_0) &\,|& (v_1 \cdot t_1) &\,|& \ldots &\,|& (v_n \cdot t_n)
\end{split}
\]
and is denoted, in `mmlang`, as either one of the expressions below, respectively.
[source]
----
[[v0;t0],[v1;t1],...,[vn;tn]]
[[v0;t0]|[v1;t1]|...|[vn;tn]]
...
There are three kinds of `polys`: `vpoly`, `tpoly`, and `tvpoly`. The `vpolys` and `tpolys` are data or process https://en.wikipedia.org/wiki/Fixed_point_%28mathematics%29[fixpoints] that exist at the extreme ends of the type/value `obj` coproduct. Between these extremes are the `tvpolys`. The `tvpolys` ultimately resolve to a `vpoly` or a `tpoly` fixpoint through the process of https://en.wikipedia.org/wiki/Compiler[compilation] or https://en.wikipedia.org/wiki/Eval[evaluation]. These `tvpolys` represent a process that hasn't terminated (https://en.wikipedia.org/wiki/State_(computer_science)[state]) or data equipped with functions (https://en.wikipedia.org/wiki/Closure_(computer_programming)[closure]).

. *Data structures* (_data_): A value-only `poly` is called a _determinate polynomial_ or `vpoly` (i.e., a `poly` lacking indeterminates).
. *Process structures* (_program_): A type-only `poly` is called an _inconstant polynomial_ or `tpoly` (i.e., a `poly` lacking constants).
. *Computational structures* (_computing_): a type/value `poly` is a `tvpoly` (i.e., a `poly` at a midpoint between a `tpoly` or `vpoly` fixpoint).
...
Determinate polynomials are degenerate in that their terms contain only values (constants). A three term `,-vpoly` whose constants are in `str` is below.
...
An empty term (zero-degree) `,-vpoly` has the following form in `mmlang`
...
A single term `,-vpoly` also uses a `.` as a terminator token.
...
| `[plus]` | `poly`    | `poly` | a right concatenation of the `poly` argument
a|
[source]
----
mmlang> ['a','b']+['c','d']
==>['a','b','c','d']
----
| `[mult]` | `poly`    | `poly` | https://en.wikipedia.org/wiki/Dot_product[dot product] (distributive over addition via https://en.wikipedia.org/wiki/FOIL_method[FOIL])
a|
[source]
----
mmlang> ['a','b']*['c','d']
==>[['a','c'],['a','d'],['b','c'],['b','d']]
----
| `[hmult]` | `poly`  | `poly`  | https://en.wikipedia.org/wiki/Hadamard_product_(matrices)[Hadamard] pairwise product
a|
[source]
----
mmlang> ['a','b']=['c','d']
==>['c','d']
----
| `[head]` |    | `obj`   | the first term of the polynomial (a https://en.wikipedia.org/wiki/List_(abstract_data_type)[list]-based interpretation)
a|
[source]
----
mmlang> ['a','b'][head]
...
----
| `[tail]` |    | `poly`  | the polynomial without the first term (a https://en.wikipedia.org/wiki/List_(abstract_data_type)[list]-based interpretation)
a|
[source]
----
mmlang> ['a','b'][tail]
...
----
| `[get]`  | `int`     | `obj`   | direct access to term by index (an https://en.wikipedia.org/wiki/Array_data_structure[array]-based interpretation)
a|
[source]
----
mmlang> ['a','b','c'].1
...
----
| `[put]`  | `int,obj` | `poly`  | insert term by index (an https://en.wikipedia.org/wiki/Array_data_structure[array]-based interpretation)
a|
[source]
----
...
.Determinate Polys and the Constant Identity Type
====
A determinate `vpoly` has terms containing only values.
...
Another interpretation of a determinate poly states that the polynomial has term types `[id]`. More specifically, for `int` constants, `int[id]`.
...
When the `tvpoly` is https://en.wikipedia.org/wiki/Polynomial#Solving_equations[solved] using `-<` and `>-`, a `vpoly` fixpoint is reached.
