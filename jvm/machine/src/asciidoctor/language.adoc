...
A `,-poly` is a classic polynomial ring with a free additive abelian group and a non-free multiplicative monoid. If \$a,b \in \tt{obj}\$ with associated quantifies \$q_i \in Q\$, then addition is defined as
...
Given the commutative nature of the abelian group, any two equal terms (ignoring quantification) can be *bulked* by summing their quantifiers using the additive operator of the corresponding quantifier ring. The https://en.wikipedia.org/wiki/Commutative_diagram[commuting diagram] below relates the 3 following `mmlang` examples, where \$\Delta\$ is `[split]` and \$\nabla\$ is `[merge]`.

image::comma-lst-tensor-example.png[align=center,width=700]
...
<2> Branch aggregation by quantifier summation is possible because the two branches are equal.
