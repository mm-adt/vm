...
| either   | `\|` | \$o+\$   | `[choose]`
|===
An mm-ADT `poly` (https://en.wikipedia.org/wiki/Polynomial[polynomial]) is an element of either a _serial_ (`;`), _parallel_ (`,`), or _choice_ (`|`) https://en.wikipedia.org/wiki/Free_object[free] https://en.wikipedia.org/wiki/Trace_monoid[trace monoid] that is https://en.wikipedia.org/wiki/Adjoint_functors[left-adjoint] to the respective \$langle \tt{obj},\ast \rangle\$, \$langle \tt{obj},+ \rangle\$, or \$langle \tt{obj},\o+ \rangle\$ monoid. The _nabla functions_, \$\nabla^{-}: \tt{poly} \rightarrow \tt{obj} \$, fold a free `poly` structure down to an `obj` via the "left-to-right" application of the associated \$\langle \tt{obj},-\rangle\$ monoid's operator.
\[
\nabla^-(\texttt{poly}) = \prod_{i} \texttt{poly}_i.
\]
Both the `-poly` and \$\langle \tt{obj},- \rangle\$ monoids derive their carrier sets from the https://en.wikipedia.org/wiki/Initial_and_terminal_objects[initial] set of mm-ADT `objs`. The injective _delta functions_, \$\Delta:\tt{obj} \rightarrow \tt{poly}\$, yield the generators of the `-poly` monoid's, where
\[
\Delta^{-}(\tt{obj}) = [\tt{obj}].
\]
Likewise, `[map]`, `[branch]`, and `[choose]` are the instruction representations of the composition of `objs`. The commuting diagrams below expose the https://en.wikipedia.org/wiki/Universal_property[universal property] of monoid mappings across these mm-ADT monoids.

image::monoid-homomorphisms.png[align=center]

Finally, the corresponding monoids across the three diagrams are the monoids of a particular encoding of the universal https://zenodo.org/record/2565243[stream ring] algebra. The `poly` monoids form a https://en.wikipedia.org/wiki/Polynomial_ring[polynomial ring] and thus, a syntactic https://en.wikipedia.org/wiki/Free_algebra[free algebra] that enables mm-ADT https://en.wikipedia.org/wiki/Metaprogramming[metaprogramming] by way of a https://en.wikipedia.org/wiki/Ring_homomorphism[ring homomorphism] from the *structural* `;,-poly` ring to the *procedural* \$\langle \tt{obj},\ast,+\rangle\$ ring.
\[
