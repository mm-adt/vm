...
[.small]
[cols="2,1,1,1",width=30%,float=left]
|===
| op | poly | obj | inst
...
| either   | `\|` | \$o+\$   | `[choose]`
|===
The mm-ADT `poly` (https://en.wikipedia.org/wiki/Polynomial[polynomial]) is both a _ctype_ and an element of either a _serial_ (`;`), _parallel_ (`,`), or _choice_ (`|`) https://en.wikipedia.org/wiki/Free_object[free] https://en.wikipedia.org/wiki/Trace_monoid[trace monoid] that is https://en.wikipedia.org/wiki/Adjoint_functors[left-adjoint] to one of the mm-ADT VM operational monoids -- \$langle \tt{obj},\ast \rangle\$, \$langle \tt{obj},+ \rangle\$, or \$langle \tt{obj},\o+ \rangle\$, respectively. Each _nabla morphism_ (\$\nabla\$) is an https://en.wikipedia.org/wiki/Epimorphism[epi] - https://en.wikipedia.org/wiki/Catamorphism[catamorphic] fold over the free `poly` structure down to an `obj` by way of a reductive, "left-to-right" application of the \$\langle \tt{obj},-\rangle\$ monoid's operator. Both the `-poly` and \$\langle \tt{obj},- \rangle\$ monoids are derived from the set of mm-ADT `objs`, where the _delta morphisms_, \$\Delta:\tt{obj}^n \rightarrow \tt{poly}\$,  *construct* `polys` and the compositional `inst` instructions (`[map]`, `[branch]`, and `[choose]`) *compute* `objs`. The commuting diagrams below expose the https://en.wikipedia.org/wiki/Universal_property[universal property] of monoid mappings across the mm-ADT monoids.

image::monoid-homomorphisms.png[align=center]

Finally, the three monoids within each monoidal category compose into the universal https://zenodo.org/record/2565243[stream ring]. The `poly` monoidal construction realizes a https://en.wikipedia.org/wiki/Polynomial_ring[polynomial ring] and thus, a syntactic https://en.wikipedia.org/wiki/Free_algebra[free algebra] enabling mm-ADT https://en.wikipedia.org/wiki/Metaprogramming[metaprogramming] via a https://en.wikipedia.org/wiki/Ring_homomorphism[ring homomorphism] from the *structural* `;,-poly` ring to the *procedural* \$\langle \tt{obj},\ast,+\rangle\$ ring -- an mm-ADT processor.

\[
;,-\texttt{poly}  \xrightarrow{\;\;\;\nabla^{;,}\;\;\;} \langle \texttt{obj},\ast,+ \rangle
\]

In terms of the mm-ADT VMs component architecture, the ring homomorphism above is a mapping from a *language* to a *processor* communicated through `objs` provided by *storage*. All mm-ADT compliant components are faithful to `obj` and the stream ring axioms that bind them. In this way, the mm-ADT VM remains forever agnostic to the specifics of the component implementations. Hence the notion that mm-ADT generates _synthetic data system_ via algebraic composition.

image::ring-homomorphisms.png[align=center,width=320]

====== Poly Structures and Processes

The `poly` serves two primary roles in mm-ADT:

. *Composite structures* (_values_): A value-based `poly` is realized as a _determinate polynomial_ (i.e., a polynomial lacking indeterminates). mm-ADT provides two convenient `poly` type constructors for working with `poly` data structures: `lst` (an `int`-indexed array) and `rec` (an `obj`-indexed array).
. *Branching processes* (_types_): . The `[split]` instruction clones the incoming `obj` across the `poly` indices and resolves each `obj` clone to an `obj` of the component type's range. For types respecting https://en.wikipedia.org/wiki/No-cloning_theorem[no-clone] axioms, `obj` coefficients are manipulated accordingly to ensure requisite `obj`/anti-`obj` https://en.wikipedia.org/wiki/Creation_and_annihilation_operators[annihilation]. All range `objs` are aggregated into a subsuming type by the `[merge]` instruction. In this manner, a type-based `poly` serves as an \$n\$-ary branch structure whose composition operator and component coefficients provide branch semantics.

====== Composite Structures

====== Branching Processes

A `poly` type is https://en.wikipedia.org/wiki/Type_constructor[constructed] and destructed using \$\Delta\$ (_delta_) and \$\nabla\$ (_nabla_), respectively. This section specifies the properties of the `polys` diagrammed below, where `,-poly` is a faithful implementation of the mm-ADT type ringoid's additive group (\$oplus\$), `|-poly` is an implementation of an https://en.wikipedia.org/wiki/Idempotence[idempotent] variation of the additive group commonly used to denote https://en.wikipedia.org/wiki/Tagged_union[sum types], and `;-poly` is a faithful implementation of the type ringoid's multiplicative monoid (\$otimes\$).
...
| `;-poly`
\[
\left(\Delta^; \circ \left(f \otimes g\right) \circ \nabla^; \right)
\]
andThen/compose-chain +
dependent slots +
product +
\$A \otimes B \implies C\$
| image:semi-delta-nabla.png[width=455,link=images/language/semi-delta-nabla.png]
|===

mm-ADT composes functions via `*` and `+`. `n`-ary functions are encoded as `poly`-coupled unary functions with the \$\Delta\$-projection (delta) serving to **copy** (clone via `,`), **split** (partition via `|`), or **compose** (pipe via `;`) a single `obj`. In particular, for the additive groupoid `polys`, their respective `poly` constructors are defined as
...
If `A` is `str` and `B` is `bool`, then \$\Delta\$ (`[delta]`) is denoted with sugar as `-<` in `mmlang`.
...
Upon exiting an instruction, all internal `poly`-branches are serialized via a \$\nabla\$-injection (nabla) which serves to  **merge** (https://en.wikipedia.org/wiki/Polynomial[polynomial addition] similar to https://en.wikipedia.org/wiki/Multiset[multiset union] via `,`), **select** (https://en.wikipedia.org/wiki/Disjoint_union[disjoint] union via `|`), or **return** (via `;`) a single stream from multiple streams. For the additive groupoids, \$nabla\$ is defined as

\[
\begin{split}
\nabla_n^,&: \overbrace{B \times \ldots \times B}^{n-\text{times}} &\rightarrow \overbrace{B + \ldots + B}^{n-\text{times}} \\
\nabla_n^,&([b,\ldots,b]) &= nb
\end{split}
\]
...
If `A` is `str` and `B` is `bool`, then \$\nabla\$ is expressed as `>-` in `mmlang`.
