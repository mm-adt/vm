...
A https://en.wikipedia.org/wiki/Cartesian_closed_category#Bicartesian_closed_categories[bicartesian closed category] has products, coproducts, a product unit, a coproduct unit, exponents, and product distributivity over coproducts with the following axioms.
...
. `|`: a _choice_ (or either) with `{n}|{m}` quantified as \$n|m\$.
. `,`: a _parallel_ (or all) with `{n},{m}` quantified as \$n+m\$.
. `;`: a _serial_ (or compose) with `{n};{m}` quantified as \$n \ast m\$.
...
mm-ADT's type system is founded on the mathematics of multi-sorted https://en.wikipedia.org/wiki/Ring_(mathematics)[rings] with https://en.wikipedia.org/wiki/Unit_(ring_theory)[unity] -- the mm-ADT *type ringoid*. A ring relates two monoids that share a carrier set. The generator of the mm-ADT type ring's carrier set is the set of all instructions -- `inst`. The multiplicative (`*`) and additive (`+`) monoids of the type ring induce the set of all mm-ADT types. Ring theory specifies the following axioms, where the associated Table <<ring-entailments>>  provides the (most well known) logical entailments of said axioms -- i.e., theorems.
