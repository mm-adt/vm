...
mm-ADT composes unary functions via `*` and `+`. `n`-ary functions are encoded as `poly`-coupled unary functions with the \$\Delta\$-projection (delta) serving to **copy** (clone via `,`), **split** (partition via `|`), or **eval** (pipe via `;`) a single `obj`. In particular, for the additive groupoid `polys`, their respective `poly` constructors are defined as
...
If `A` is `str` and `B` is `bool`, then \$\Delta\$ is expressed as `-<` in `mmlang`.
...
Upon exiting an instruction, all internal `poly`-branches are serialized via a \$\nabla\$-injection (nabla) which serves to  *merge* (https://en.wikipedia.org/wiki/Union_(set_theory)[union] via `,`) or *select* (https://en.wikipedia.org/wiki/Disjoint_union[disjoint] union via `|`) many independent streams \$nabla\$ is defined as
...
image::comma-delta-nabla.png[width=455,float=left]

image::pipe-delta-nabla.png[width=465,float=right]
