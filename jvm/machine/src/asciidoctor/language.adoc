...
There are two distinct kinds of mm-ADT `objs`: _quantified_ type objects and _quantified_ value objects. The products of the `obj` sum are called *type* and *value*. That is the *obj meta-model*.
...
NOTE: image:type-value-trace-short.svg[float=right,width=350] In the diagram above, the type vertices are elements of a https://en.wikipedia.org/wiki/Free_algebra[free algebra] called the `inst` monoid.
