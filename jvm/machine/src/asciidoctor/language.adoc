...
image:bubble-chamber.png[width=200,float=left] The equation above realizes a structure and process joyfully named the https://en.wikipedia.org/wiki/Bubble_chamber["bubble chamber"]. In experimental higher-energy physics, a bubble chamber small room fill with vapor under high pressure, where particles are shot into the room and the trace they leave through the vapor (their _varpor trail_) provides physicists insight into the nature of the particle -- weight, speed, rotation, etc. In mm-ADT, given the equation above, \$x\$ plays the role of the _particle_ and `;-poly` the _bubble chamber_ with each term in the `;-poly` acting as a _vapor droplet_.
