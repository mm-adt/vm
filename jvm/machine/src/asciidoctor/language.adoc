...
The mm-ADT VM is written in https://scala-lang.org/[Scala]. The core library exposes a fluent program builder https://en.wikipedia.org/wiki/Application_programming_interface[API] called `mmscala`. Most compiler designers should avoid `mmscala` as direct access to the https://en.wikipedia.org/wiki/Java_virtual_machine[JVM] can create unsafe mm-ADT programs. Instead, `mmlang` should be the target language for most compilers.
...
This internal structure is well-defined as an https://en.wikipedia.org/wiki/Ring_%28mathematics%29[algebraic ring]. The https://en.wikipedia.org/wiki/Ring_%28mathematics%29#Definition[ring axioms] specify how the internals of an `obj` are related via two binary operators: \$\times\$ and \$\+\$ . One particular ring axiom states that products https://en.wikipedia.org/wiki/Product_(category_theory)#Distributivity[distribute] over co-products. Thus, the previous formula can be rewritten as
\[
\texttt{obj} = (\texttt{type} \times \texttt{q}) + (\texttt{value} \times \texttt{q}).
\]
This representation, which is https://en.wikipedia.org/wiki/Isomorphism[isomorphic] to the previous, states two distinct kinds of mm-ADT `objs`: *quantified types* and *quantified values*. Some example mm-ADT `objs` (types and values) are presented below in `mmlang`.
...
Many modern programming environments make a sharp distinction between types and values. Each finds its prominent use in either compilation (types) or evaluation (values). In mm-ADT, at the highest level, there are `objs` and they __refer to__ other `objs` such that

* *Types* refer to values and
* *Values* refer to themselves.

This definition eases many longstanding distinctions in computer science. The table below presents a collection of the more interesting conceptual blurs that mm-ADT introduces. The particulars of each will be discussed over the course of this documentation.
...
Types and values both have a *ground* that exists outside of the mm-ADT VM within the hosting environment (e.g. the JVM). The ground of the mm-ADT value `2` is the JVM primitive `2L` (a Java `long`). The ground of the mm-ADT type `int` is the JVM class `java.lang.Long`. When the instruction `[plus,4]` is applied the mm-ADT `int` value `2`, a new mm-ADT `int` value is created whose ground is `6L`. When `[plus,4]` is applied to the mm-ADT `int` type, a new type is created with the same `java.lang.Long` ground as the previous type. Thus, the information that distinguishes `int` from `int[plus,4]` is in the remembrance of the instruction that was applied to get from `int` to `int[plus,2]`. For a type, this history is called the *type definition*.

NOTE: In practice,the string representation of a value is it's ground and the string representation of a type is it's instruction history.

image::type-value-trace.png[align=center,width=550]

Both types and values exist in a larger graph called a *process trace graph*. In theory, the complete history of an mm-ADT program from compilation to execution is stored in this graph. However, in practice, the mm-ADT VM removes an `obj` trace (a path in the graph) when the program will not require it in the future.
...
=== The Instruction Set Architecture
 The abstract algebra axiomatizing this process is best captured as a https://en.wikipedia.org/wiki/Monoid[monoid] called the *inst monoid*.
==== The Inst Monoid
