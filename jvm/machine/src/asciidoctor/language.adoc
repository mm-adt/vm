...
mm-ADT's type system is founded on a multi-sorted https://en.wikipedia.org/wiki/Ring_(mathematics)[ring] with https://en.wikipedia.org/wiki/Unit_(ring_theory)[unity] called the mm-ADT *type ringoid*. A ring is composed of a multiplicative monoid (`\*`) and an additive group (`+`) that share the same carrier set. A https://ncatlab.org/nlab/show/Ab-enriched+category[ringoid] generalizes the mathematics of a ring for multi-sorted carrier partitions (i.e. for typed elements). The mm-ADT type ringoid https://en.wikipedia.org/wiki/Generator_(mathematics)[generator] is the set of all instructions (`inst`), where, with the ringoid's multiplicative operator (`*`) and commutative additive operator (`+`), when faithfully applied according to sort, induce the set of all possible mm-ADT types.
...
mm-ADT `polys` (polynomials) are constructed and destructed using \$\Delta\$ and \$\nabla\$. For the additive groupoid `polys`, the diagrammatic representation of the relationship between \$\Delta\$ and \$\nabla\$ is provided below and will be analyzed in this section.

[cols="3,^8"]
|===
| `,-poly` =
\$\left(\Delta^, \cdot \langle f,g \rangle \cdot \nabla^,\right)\$  | image:comma-delta-nabla.png[width=455]
| `\|-poly` = \$\left(\Delta^\| \cdot \langle f,g \rangle \cdot \nabla^\|\right) \$ | image:pipe-delta-nabla.png[width=465]
|===
...
In mmlang, a `poly` can be denoted as a _structure_ using `[ ]` (a value) or as a _process_ using `< >` (a type). The latter has the following desugaring derivation.
