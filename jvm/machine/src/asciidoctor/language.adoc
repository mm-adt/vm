...
It is through "_quantified objects_" that mm-ADT types and values, which are typically disjoint structures in most programming environments, co-exist at the same level of abstraction (within the same category).

* A type refers to values.
* A value is a type that only refers to itself.

From the unification of types and values, many other seemingly disparates aspects of computing blend together naturally. The particulars of the unifications itemized below will be detailed in respective sections of this document.
...
|functions   | types      | functions are types whose values are realized at runtime.
|program     | type       | a program is "complicated" type.
...
* Streams may contain duplicate objects (not like a set).
