...
. *sum composites*: the symbols `,` (_orThen_) and `|` (_xorThen_) relate terms by a function with the general form \$ \tt{obj} = \sum_{i<n} \tt{v}_i;\tt{t}_i \$.
...
While mm-ADT supports the full spectrum of polynomial forms, there are two significant forms that are at the extreme ends of the type/value `obj` divide and they function to either structure data or processes.

. *Data structures* (_data_): A value-only `poly` is called a _determinate polynomial_ (i.e., a `poly` lacking indeterminates).
. *Process structures* (_program_): A type-only `poly` is called an _inconstant polynomial_ (i.e., a `poly` lacking constants).
...
Determinate polynomials are degenerate in that their terms contain only values (constants). A three term determinate whose constants are in `str` is below.
...
An empty term (zero-degree) `,-poly` has the following form in `mmlang`
...
A single term `,-poly` also uses a `.` as a terminator token.
...
The reason for `.` is that the `,-poly` below is a two term poly with the second term being `obj{0}`.
...
|===
| inst     |  arg(s)   | range  | description | mmlang example
...
.A Note on Determinate Polys and the Identity Type
====
The degenerate determinate `poly` is composed of value only terms.
...
Another interpretation is a `poly` whose term types are always `[id]`.
...
When the `poly` is solved using `-<` and `>-`, the determinate form is realized.
