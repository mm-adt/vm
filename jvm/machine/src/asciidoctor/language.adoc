...
\begin{cases}
  ...
  \prec a\{ q_0 \},b\{ q_1 \} \succ & \text{otherwise},
\end{cases}
\]
where the \$+\$ between quantifiers denotes the quantifier ring's additive operator (typically `[plus]` for `int` quantifiers) and \$\prec \tt{obj},\ldots,\tt{obj} \succ \$ denotes `obj` addition (stream `[split]`/`[merge]`). Given the https://en.wikipedia.org/wiki/Commutative_property[commutative nature] of the `,-poly` abelian group, any two equal objects in \$\tt{object}\$ can be *bulked* by summing their quantifiers. The https://en.wikipedia.org/wiki/Commutative_diagram[commuting diagram] below depicts the equalities of the following `mmlang` examples, where \$\Delta\$ is `[split]` and \$\nabla\$ is `[merge]`.
