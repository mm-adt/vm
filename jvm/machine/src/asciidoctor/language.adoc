...
| _merge_     | `>-`        | \$\nabla\$      | linear combine
| _branch_    | `[ ]`       | \$◊\$           | scalar apply combine
| _combine_   | `=`         | \$\cdot\$       | `poly` pairwise combine
...
.3+| `lst` | `,`  | all                 | https://en.wikipedia.org/wiki/Union_(set_theory)[union]
           ...
           |  `;` | last                | https://en.wikipedia.org/wiki/Method_chaining[chain]
.3+| `rec` | `,`  | all match key       | https://en.wikipedia.org/wiki/Conditional_(computer_programming)[condition]
...
3-<([plus,1];[mult,2])>-
----
|The "_branch_"-pattern (`-<()>-`) is common enough to warrant a single, unifying instruction called `[branch]` and a corresponding sugar of `[poly]`. If the evaluating processor encounters `[branch]`, it can compute the instruction using a `poly` of streams versus a stream of `polys`. The difference is significant in distributed environments, where the former enables decoupled, non-communicated parallel pipelines and the latter requires coupled, potentially-communicating parallel pipelines.
a|
[exec]
----
3[[plus,1],[mult,2]]
...
. *object semiring* \$(\tt{object},=>,◊, \_{_1} , \__{0} ) \$ and
...
====== Universal Commutativity of Coefficients

[cols="7,6"]
|===

| This expression is equivalent to `obj{0}`. This can be provided by applying the `;-poly` coefficient equation. `2*3*0 = 2*0*4 = 0*3*4`. In general, if there exists a *0*-quantified `obj` in a `obj`  monoid expression, then the result is always `obj{0}`.
a|
[exec]
----
...
| If the quantifier ring is not commutative, it is possible to propagate coefficients left or right through an `obj` `\=>` expression. Likewise, unless the quantifiers are https://en.wikipedia.org/wiki/Prime_element[prime elements], quantifier propagation need not preserve the factors of a `*`. In this way, if the geometric series remains the same, any quantifier distribution is legal.
a|
[exec]
----
6{2}[plus,1]{3}[plus,2]{4}
...
| If the quantifier ring is not commutative, it is possible to propagate coefficients left or right through an `obj` `\=>` expression.
a|
[exec]
----
6[plus,1]{6}[plus,2]{4}
...
| Quantifiers propagate along the the multiplicative `obj` monoid via their `\*`-operator. They propagate along the additive `obj` group via their `+`-operator. In this way, if two branches have have https://en.wikipedia.org/wiki/Orthogonality[orthogonal] quantifiers of the same magnitude, then when they leave the `+`-group to be additively merged onto the `*`-monoid, they cancel each other out. Various set theoretic and quantum operations make use of constructive and deconstructive quantifier https://en.wikipedia.org/wiki/Orthogonality[interference] when computing.
a|
[exec]
----
