...
3-<([plus,1];[mult,2])>-
----
|The "_split-merge_"-pattern (`-<()>-`) is common enough to warrant a single, unifying instruction called `[splitmerge]` and a corresponding sugar of `[poly]`. If the evaluating processor encounters `[splitmerge]`, it can compute using a `poly` of streams versus a stream of `polys` as may be required with `[split]`/`[merge]`. The difference is significant in distributed environments, where the former supports decoupled, non-communicated parallel pipelines and the latter requires coupled, potentially-communicating parallel pipelines.
a|
[exec]
----
3[[plus,1],[mult,2]]
...
Finally, the most compact sugar forms of the running "_split-merge_"-examples is presented below. Given the verbosity of branching expressions, all subsequent `mmlang` examples will favor sugar constructs.
a|
[exec]
----
3[+1,*2]
...
A `,-poly` is a classic https://en.wikipedia.org/wiki/Polynomial_ring[polynomial ring] maintaining a free additive https://en.wikipedia.org/wiki/Abelian_group[abelian group] and a non-free multiplicative https://en.wikipedia.org/wiki/Monoid[monoid]. The mm-ADT `obj` is the  https://en.wikipedia.org/wiki/Product_ring[direct product]  of two rings:

. *object ring* \$\langle \tt{object},=>,[ ] \rangle \$ and
. *quantifier ring* \$\langle Q, \ast, + \rangle \$.

If \$a,b \in \tt{object}\$ and \$q_0, q_1 \in Q\$ are the elements of the product pairs denoting an `obj`, then the additive operator of the `obj` ring is defined as
\[
[a_{q_0},b_{q_1}] =
\begin{cases}
...
\end{cases}
\]
where the \$+\$ between quantifiers denotes the quantifier ring's additive operator (typically `[plus]` for `int` quantifiers) and \$[\tt{obj},\ldots,\tt{obj}]\$ denotes `obj` addition (stream `[split]`/`[merge]`). Given the https://en.wikipedia.org/wiki/Commutative_property[commutative nature] of the `,-poly` abelian group, any two equal objects in \$\tt{object}\$ can be *bulked* by summing their quantifiers.

When evaluating a \$x \in \tt{obj}\$, the additive `,-poly` serves as an algebraic https://en.wikipedia.org/wiki/Semigroup_action[action] satisfying the equation below. From another perspective, the `,-poly` is a left https://en.wikipedia.org/wiki/Module_(mathematics)[module] and \$x\$ a scalar in this generalized scalar multiplication.
...
The https://en.wikipedia.org/wiki/Commutative_diagram[commuting diagram] below depicts the equalities of the following `mmlang` examples, where \$\Delta\$ is `[split]` and \$\nabla\$ is `[merge]`.
...
'mar'[plus,'ko']{2}               //<3>
----
<1> The `'mar'` `str` is copied (\$ \prec \$) and applied (\$ => \$) to each `[plus,'ko']` polynomial term and then aggregated (\$ \succ \$).
...
['b','c']+'a'         //<2>
----
<1> The value `a` is split (\$\prec\$) and applied (\$=>\$) to `[plus,'b']` and `[plus,'c']` elements before being merge (\$\succ\$).
<2> The values `a` and `b` are merged (\$\succ\$) and then applied (\$=>\$) to `[plus,'a']`.

Polynomial factorization plays an important role in type optimization (the compiler), where, for example, with considerations to distributed computing:
