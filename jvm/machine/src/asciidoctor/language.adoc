...
The study of polynomials makes a distinction between https://en.wikipedia.org/wiki/Univariate[univariate] and https://en.wikipedia.org/wiki/Polynomial#Multivariate_polynomial[multivariate] polynomials, where if all the _object_ components of the `obj` product are equal, then the polynomial is univariate, else it is multivariate. In mm-ADT, multivariate `polys` are much more common than univariate `polys`. However, a primary function of the mm-ADT optimizing compiler is the determination of term equivalence via term rewrites so as to *bulk* terms using the additive and multiplicative operators of the expression's associated quantifier ring (i.e. the ring of coefficients).
