...
mm-ADT `polys` (polynomials) are `objs` composed in a non-reductive manner from other `objs`, where a specified algebra relates the components of the composition and the meaning of the polymorphic `[mult]` and `[plus]` instructions: _parallel_ (`,`), _serial_ (`;`) , and _choice_ (`|`). A `poly` serves two primary roles:

. *Composite structures* (_values_): The base ctypes are fundamental, atomic datum. Everything else is a `poly`. mm-ADT provides two convenient `poly` structures for which all other composites are founded. A `lst` is a polynomial list. A `rec` is a polynomial key linked to a polynomial value.
. *Branching processes* (_types_): When an `obj` is passed as an argument to an additive `poly` group, the `obj` is split across the individual `poly` slots, where if the slot contains a type, the `obj` is domain and the result of the branch is the range. Given `poly` `[head]/[plus]`-style instructions, `polys` enable https://en.wikipedia.org/wiki/Metaprogramming[metaprogramming] in mm-ADT.
...
NOTE: An mm-ADT base type is also realized as a polynomial with a single indeterminate and coefficient. As an example, `'mmadt'{2}`.
