...
[cols="^1,^1",width=90]
|===
| Module Algebra                                      | mm-ADT Algebra

2.+^h| Right \$X\$-Module Axioms
| \$x \cdot (a +_A b) = (x \cdot a) +_A (x \cdot b) \$ | \$x \Delta (a,b) \mapsto (x=>a,x=>b)\$
| \$(x +_X y) \cdot a = (x \cdot a) +_A (y \cdot a) \$ | \$(x,y) \nabla a \mapsto (x=>a,y=>a) \$
| \$(x \ast_X y) \cdot a = x \cdot (y \cdot a)\$       | \$(x;y) \nabla a \mapsto x \Delta (y;a)\$
| \$\mathbf{1}_X \cdot a = a\$                         | \$\underline\mathbf{1} => a \mapsto a\$
2.+^h| Right \$X\$-Module Axioms
| \$(a +_A b) \cdot x = (a \cdot x) +_A (b \cdot x) \$ | \$(a,b) \nabla x \mapsto (a=>x,b=>x)\$
| \$a \cdot (x +_X y) = (a \cdot x) +_A (a \cdot y) \$ | \$a \Delta (x,y) \mapsto (a=>x,a=>y) \$
| \$a \cdot (x \ast_X y) = (a \cdot x) \cdot y\$       | \$a \Delta (x;y) \mapsto (a;x) \nabla y \$
| \$a \cdot \mathbf{1}_X = a\$                         | \$a => \underline\mathbf{1} \mapsto a\$
|===
