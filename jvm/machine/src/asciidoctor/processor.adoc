...
[cols="^1,^1",width=75,align=center]
|===
| Module Algebra                                      | mm-ADT Algebra

2.+^h| Left \$X\$-Module Axioms
| \$x \cdot (a +_A b) = (x \cdot a) +_A (x \cdot b) \$ | \$x \Delta (a,b) = (x=>a,x=>b)\$
| \$(x +_X y) \cdot a = (x \cdot a) +_A (y \cdot a) \$ | \$(x,y) \nabla a = (x=>a,y=>a)\nabla \$
| \$(x \ast_X y) \cdot a = x \cdot (y \cdot a)\$       | \$(x;y) \nabla a = x => y => a\$
| \$\mathbf{1}_X \cdot a = a\$                         | \$\underline\mathbf{1} => a = a\$
2.+^h| Right \$X\$-Module Axioms
| \$(a +_A b) \cdot x = (a \cdot x) +_A (b \cdot x) \$ | \$(a,b) \nabla x = (a=>x,b=>x)\nabla \$
| \$a \cdot (x +_X y) = (a \cdot x) +_A (a \cdot y) \$ | \$a \Delta (x,y) = (a=>x,a=>y) \$
| \$a \cdot (x \ast_X y) = (a \cdot x) \cdot y\$       | \$a \Delta (x;y) = (a=>x;a =>x=>y) \$
| \$a \cdot \mathbf{1}_X = a\$                         | \$a => \underline\mathbf{1} = a\$
|===
