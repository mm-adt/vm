...
Due to the left and right distributive ring axioms, the following derivation yields the following equivalence.
...
====== Modules of the Type Ringoid

Free elements are lexicographically one dimensional--lines. The type ringoid's carrier set is the union of the free `obj` stream ring's additive abelian group and multiplicative monoid. Thus, with two free magmas, the elements of the type ringoid are two dimensional--rectangles. In `mmlang`, the horizontal axis is denote `[;]` and the vertical axis is denoted `[,]`. For example, the following type is two-dimensional. The original reducible free polynomial rings are expressed in the free type ringoid using `[,]` and `[;]`.
...
In a manner analogous to polynomials in linear algebra, these free polynomials can be organized into a matrix where the multiplicative identity `_` is used to pad the first two rows to ensure an \$n \times m\$-matrix.
...
A left `obj`-module can be factored out of the matrix leaving an expression of the form \$\mathbf{v}^{\top} \mathbf{M}\$.
...
In an analogous manner, a right scalar `obj`-module can be factored out leaving an expression of the form \$\mathbf{v}^{\top} \mathbf{M} u \$.
...
This factored form can then be evaluated via `obj` scalar left multiplication.
...
Again, to be certain, all 3 forms yield the same result.
...
The linear algebraic type ringoid compartmentalizes each type induced by an instruction. This completely free representation is staged for manipulation via matrix operations and ultimately, during execution, provides a 2-dimensional circuit diagram of the computational pipeline convenient for distributing across machines of a cluster and/or threads of a machine.
