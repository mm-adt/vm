...
machine (VM), data processing technologies can be composed to create novel _synthetic data systems_ tailored to the
space/time constraints of the problem at hand. Furthermore, each integration point offers a development team access
to a revenue stream from the commercial organizations leveraging the technologies in the mm-ADT ecosystem.
...
Serial composition is multiplication (`\*`) and parallel composition is addition (`+`). If `<A,+,*>` is a ring with `A` the carrier set, `0` the
additive identity, `1` the multiplicative identity, and `a,b,c,...,0,1` in `A`,
then the ring must obey the itemized axioms and benefit from the deductions that yield the common implications and equalities.
...
All serial and parallel compositions are https://en.wikipedia.org/wiki/Associative_property[associative] except for those that
use instructions from the `inst` sub-ring: _the reduce near-ring_ (discussed later). Finally, the
...
Typically, a type denotes a set of elements. A set is a non-repeating collection of said elements.
A _stream_ generalizes a set. A stream is an unordered collection of `objs` of varying cardinality (a multi-set, or bag).
In mm-ADT, `int` is shorthand for `int{1}`--a single integer. This is analogous to other languages, where when `int` is used
to define a value, it is referring to a single integer from the set of all integers--e.g., `val answer:int = 42`.
Where mm-ADT differs from other languages is that `bool{42}` is a type denoting `42` boolean values. All
`mmlang` expressions within `{ }` operate on the mm-ADT VM's *_quantifier ring_* (w/ unity). Any `obj` that supports
`+`/`*`/`0`/`1` and the axioms governing their composition is supported by mm-ADT. In mm-ADT, types can be specified to
...
that are usually captured by typeclasses or functors (via lifting) are expressed in mm-ADT via quantification.
...
over the _quantifier_ and _instruction_ rings. The direct product of these two rings is a ring itself. Thus, mm-ADT's
types are founded on ring theory.
...
A _model_ is a set of related types (a type of types). Every mm-ADT model forms a category where the category's _objects_
are the canonical types and the _morphisms_ are types that span the canonicals. All mm-ADT types respect associative composition
and every type's identity morphism is the `[id]` instruction as in `int<=int[id]`.

[ditaa,"type-commutativity",align="center"]
....
         int---[gt,0]---->bool

          |                ^
          |                |
[plus,int[is,[gte,0]]      |[id]
          |                |
          v                v
