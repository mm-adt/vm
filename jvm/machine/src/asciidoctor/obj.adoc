...
\[
\begin{split}
a \prec &\; (x_0,x_1,\ldots,x_n) \;&= (ax_0,ax_1,\ldots,ax_n) \\
a \prec &\; (x_0;x_1;\ldots;x_n) \;&= (ax_0;x_1;\ldots;x_n)   \\
a \prec &\; (x_0|x_1|\ldots|x_n) \;&= (ax_i),
\end{split}
\]
where \$a \prec \tt{poly}\$ is the instruction `a \=> [split,poly]`. The `[merge]` instruction evaluates the `poly` according to the algebra denoted by its term separator (`,`, `;`, or `|`). This has the effect of "draining" the `poly` of it's internal `objs` such that
\[
\begin{split}
(ax_0,ax_1,\ldots,ax_n) \succ  \;&= \left( \coprod_{i=0}^n a \Rightarrow x_i \right) \\
(ax_0;x_1;\ldots;x_n)   \succ  \;&= \left( a \Rightarrow \prod_{i=0}^n x_i \right)   \\
(ax_i)                  \succ  \;&= ax_i : x_i \neq \mathbf{0},
\end{split}
\]
...
\[
\begin{split}
a \prec &\; (x_0,x_1,\ldots,x_n) \succ  \;&= \left( \coprod_{i=0}^n a \Rightarrow x_i \right)  \;&= a \Rightarrow \begin{bmatrix} x_0 \\ x_1 \\ \vdots \\ x_n \end{bmatrix} \\
a \prec &\; (x_0;x_1;\ldots;x_n) \succ  \;&= \left( a \Rightarrow \prod_{i=0}^n x_i \right)    \;&= a \Rightarrow  \begin{bmatrix} x_0 \\ x_1 \\ \vdots \\ x_n \end{bmatrix} \\
a \prec &\; (x_0|x_1|\ldots|x_n) \succ  \;&= ax_i : x_i \neq \mathbf{0}                        \;&= a \Rightarrow \begin{bmatrix} x_0 \\ x_1 \\ \vdots \\ x_n \end{bmatrix} \\
\end{split}
\]
