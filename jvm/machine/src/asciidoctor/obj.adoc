...
A `poly` is a https://en.wikipedia.org/wiki/Free_object[free object]. Free objects are https://en.wikipedia.org/wiki/Universal_algebra[universal] structures in that they respect the equations of an abstract algebraic structure. However, the term _free_ refers to the fact that these objects are "free" from the equational constraints of a concrete algebra. The following table presents examples of these two classes of equations. If the concrete algebra equations appear random, it is because they are. Each instance of an abstract algebra's operator(s) map elements to elements in a manner specific to the domain application.
...
In mm-ADT, a `poly` can be used as a https://en.wikipedia.org/wiki/Collection_%28abstract_data_type%29[*collection*] data structure, where the collection's semantics are tied to the operator(s) of the free object abstract algebra. Moreover, type-based `polys` are leveraged as https://en.wikipedia.org/wiki/Control_flow[*control*] structures. The mm-ADT `poly` is a versitile structure because it remains agnostic to the types and values contained therein while remaining consistent with the https://zenodo.org/record/2565243[stream ring algebra]'s operators' axioms and entailed theorems.
