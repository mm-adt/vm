...
[exec,exception=LanguageException]
----
36 => int[define,nat<=int[is>0]]                   <1>
36 => nat<=int[define,nat<=int[is>0]]              <2>
36 => nat<=int[define,nat<=int[is>0]][mult,-1]     <3>
----
...
The `mm` model-ADT is too simple to be informative. The complexity of its types exist outside the virtual machine. In order to provide a comprehensive understanding of mm-ADT models, the following sections will build a *property graph* model-ADT (`pg`) in stages starting with `pg_1`, then `pg_2`, so forth before reaching the final complete encoding of `pg`.

==== Types
[discrete]
===== Property Graph Model 1

A property graph, like all graphs, are composed of vertices and edges. Both vertices and edges have key/value pair attributes called _properties_. In the `pg_1` model below, a vertex has an `id` property with an `int` value. An edge is directed and binary. It has an outgoing/start vertex (`outV`) and an incoming/end vertex (`inV`). The `pg_1` model's associated diagram is not a commuting diagram and the `.` prefixes on the `inst` morphisms denote the `mmlang` sugar notation for `[get]`, where `.outV` is sugar for `[get,'outV']`.

[.center]
[cols="9,5",width=100,grid=none,frame=none]
|====
a|
[source]
----
...
A few examples of the `pg_1` are presented below.

[exec,exception=LanguageException]
----
:[model,pg_1]                                //<1>
1 => vertex<=int-<('id'->_)                  //<2>
1 => vertex<=int-<('id'->_,'age'->+28)       //<3>
1 => vertex<=int-<('ID'->_)                  //<4>
----
<1> The defined type prefix uses `[model]` to load the `pg_1` model into the `obj` graph.
<2> A `vertex` is constructed by copying an `int` to the value of the `id` field.
<3> Extraneous (non-ambiguous) data is mapped to the terminal \$\mathbf{0}\$.
<4> If the mapping yields a structure that can not be coerced into a `vertex`, then the program type is ill defined.

An edge can be constructed in any number of ways. The example below takes the `int` product `(int;int)`(\$\mathbb{Z} \times \mathbb{Z}\$) to `edge`.

[exec]
----
:[model,pg_1]
(1;2) => edge<=(int;int)-<('outV'->-<('id'->.0),'inV'->-<('id'->.1))
----

The line above reads: Given the `;-lst` product pair `(1;2)`, *split* (`-<`) the pair across a two element `rec`, where their values are single element `recs` which again split the pair across the two fields (`outV` and `inV`) and project out a unique `int` id to each field value. Finally, given `edge\<=(int;int)` a final coecion to `edge` is required. It is the role of the mm-ADT VM to compute the most efficiet way to accomplish this. For this example, simply labeling `rec` pair elements `vertex` and the outer `rec` `edge` is sufficient to complete the transformation from `(int;int)` to `edge` given `pg_1`.
...
5 => vertex                                                                //<1>
(5;6) => (vertex;vertex)                                                   //<2>
(5;6) => (vertex;vertex) => edge                                           //<3>
(5;6) => edge                                                              //<4>
5-<(vertex;vertex)=>edge                                                   //<5>
----
<1> An `int` value morphed into a `vertex`.
<2> An `int` value pair morphed into a `vertex` pair (a *type edge*).
<3> An `int` value pair morphed into a `vertex` pair and then into an edge (two *type edges* composed to a *type path*).
<4> An `int` value pair morphed into an edge (a *type path*).
<5> An `int` value split into `vertex` clone pairs and then morphed into a https://en.wikipedia.org/wiki/Loop_(graph_theory)[self-loop] edge.
