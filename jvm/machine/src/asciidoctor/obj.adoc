...
===== Poly Types
A `poly` is a https://en.wikipedia.org/wiki/Free_object[free object]. A free object imposes the equations of the abstract algebraic structure it captures (e.g. an https://en.wikipedia.org/wiki/Abelian_group[abelian group] or a https://en.wikipedia.org/wiki/Monoid[monoid]), but not the equations of the elements of a concrete instance of that algebra (e.g. \$a \ast b = c\$ or \$a + a = a\$). From a https://en.wikipedia.org/wiki/Data_structure[data structure] perspective, a free object is a https://en.wikipedia.org/wiki/Collection_%28abstract_data_type%29[collection] with semantics strictly tied to the abstract algebra's operator semantics, not its element semantics. That is, free object *collection structures* respect identity, inverse, associativity, and commutativity. The entailment is that common collections such as https://en.wikipedia.org/wiki/List_(abstract_data_type)[lists] and https://en.wikipedia.org/wiki/Multiset[multisets] are realized by free object `polys`. Moreover, when the `poly` elements are not values, but types, common *control structures* are realized such as https://en.wikipedia.org/wiki/Switch_statement[switch] and https://en.wikipedia.org/wiki/Null_coalescing_operator[coalesce]. The mm-ADT `poly` exploits the https://en.wikipedia.org/wiki/Duality_(mathematics)[duality] between types and values at the level of a free object.
