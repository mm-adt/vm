...
\end{cases}
\]
such that when two term objects are not equal, the `poly` is https://en.wikipedia.org/wiki/Irreducible_representation[irreducible]. The lack of order along with quantifier "weighting," `,-poly` collections realize https://en.wikipedia.org/wiki/Multiset[multiset] semantics (also known as bags or weighted sets).

.;-poly Collections
The *;-polys* capture the non-commutative, multiplicative https://en.wikipedia.org/wiki/Monoid[monoid] of the mm-ADT ring. Due to non-commutativity, the `;` deliminated terms are ordered. The consequence is a collection data structure with https://en.wikipedia.org/wiki/List_(abstract_data_type)[list]-semantics.

.|-poly Collections
The *|-polys* capture a non-commutative additive group.
