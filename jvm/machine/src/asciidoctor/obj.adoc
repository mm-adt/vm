...
[.small]
[cols="2,1,2" width=45, float=right]
|====
| idiom                                                                      | inst           | description

| https://en.wikipedia.org/wiki/Variable_%28computer_science%29[variables]   | `[to]/[from]`  | `obj` references
...
It is through *multiple distinct identities* in `inst` that mm-ADT supports the programming idioms in the associated table. In particular, state stored in the path history of the `obj`.
...
mm-ADT's *multiple identity instructions* simply compute \$f(x) \mapsto x\$, but as edge labels in the `obj` graph, they store state information that can be later accessed via trace-based path analysis (i.e. via `[path]`).
