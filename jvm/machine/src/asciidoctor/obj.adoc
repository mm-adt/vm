...
A *definition* takes the familiar form
\[
a<=b,
\]
or
...
a:b
\]
where \$a\$ is _determined_ by \$b\$ and \$a:b \cong a<=b\$ is used to define new ctypes (i.e., a type definition that requires no instructions to perform that mapping). Analogous phrases are: \$a\$ is a _function_ of \$b\$ or \$a\$ is _dependent_ on \$b\$, and for the latter \$a:b\$ encoding, \$a\$ is _structurally equivalent_ to \$b\$.

It is no coincidence that every mm-ADT program (type) has this form as every mm-ADT program is a type definition. For most of the documentation, the examples have been from within the `mm` model-ADT where there are 6 types: `bool`, `int`, `real`, `str`, `lst`, and `rec`. It is possible create new types that are ultimately _grounded_ (Cayley rooted) in the `mm` model-ADT types. This is the purpose of the `[define]` instruction.

[exec]
----
1 => [define,vertex:('id'->int), %
             edge:('outV'->vertex,'inV'->vertex)] %
     -<edge:('outV'->-<vertex:('id'->_),'inV'->-<vertex:('id'->+1))
----

The `[define]` instruction is placed at the start of the type (the start of the desired definition scope). Such prefixing can happen automatically by way of a *type prefix*. The type prefix is a generalization of an `import` or `module` https://en.wikipedia.org/wiki/Comparison_of_programming_languages_(syntax)#Libraries[library statement] found in other programming languages. The generalization is that a type prefix can be any type. It is simply concatenated to the program type prior to compilation (as ruled by the free `inst` monoid type composition algebra to be detailed soon).

[exec,exception=LanguageException]
----
:[model,mm][define,vertex:('id'->int), %
                   edge:('outV'->vertex,'inV'->vertex)]               //<1>
1 => int-<edge:('outV'->-<vertex:('id'->_), %
                 'inV'->-<vertex:('id'->+1))                          //<2>
1 => int-<edge:('outV'->-<vertex:('bad_id'->_), %
                 'inV'->-<vertex:('bad_id'->+1))                      //<3>
1 => int-<edge:('outV'->-<vertex:('id'->_), %
                 'inV'->-<vertex:('id'->+1),'extra'->'data')          //<4>
----
<1> The `mm` model-ADT is extended with a `vertex` and an `edge` definition.
<2> The `vertex` and `edge` type definitions are automatically prepended to all subsequent programs.
<3> If there is no mappig from the start `obj` to the type definition, the `obj` is incorrectly typed.
<4> If an `obj` has extraneous data, it is still a valid type as it can be mapped to the type definition.
