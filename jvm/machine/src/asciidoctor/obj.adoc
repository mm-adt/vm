...
:[model,mm]                                               %%
 [define,vertex:('id'->int),                              %                      //<1>
         ...
         vertex<=int-<('id'->_),                          %                      //<3>
         loop<=vertex-<([id];[id]),                       %                      //<4>
         edge<=(vertex;vertex)-<('outV'->.0,'inV'->.1)]                          //<5>
5
5 => vertex
5 => vertex => loop
5 => vertex => loop => edge                                                      //<6>
5 => loop => edge                                                                //<7>
----
image::digraph-type-path-1.svg[width=400,align=center]
...
<4> A path from a `vertex` to a 2-tuple `;-lst` named `loop`.
<5> A path from a 2-tuple `vertex` `;-lst` to a `edge.
<6> A fine-grained *type path* start at an `int` and ending at the `edge` materialization of the `int`.
<7> A course-grained *type path* from `int` to `edge` via `loop`.
