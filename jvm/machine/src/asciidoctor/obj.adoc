...
The `[barrier]` operator computes an output based on the entire input. This _blocking_ behavior is used for link:https://en.wikipedia.org/wiki/Reduction_Operator[reduce]/link:https://en.wikipedia.org/wiki/Fold_%28higher-order_function%29[fold]-based computations. The \$1\$ subscript is a particular augmentation to the higher-order \$\oplus\$ operator, where \$\oplus_1\$ operator returns the first non-zero `obj` -- i.e., the head of the stream (a https://en.wikipedia.org/wiki/Lazy_evaluation[lazy computation]).
...
`|-poly` yields singleton `lsts` and `recs`. This seemingly odd behavior proves its importance when considered `|-polys` in the context of flow controls.
