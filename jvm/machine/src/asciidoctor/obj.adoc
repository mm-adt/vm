...
NOTE: A type definition with no instructions serves as both a model constructor and canonical type (a ctype). As a canonical type, the path from source to target does nothing. A value that matches the left-hand side is simply named with the label on the right-hand side.
\[
\tt{(id->int)}\;\;\tt{\textrm{—[noop]}{\longrightarrow}}\;\;\tt{vertex}.
\]


Three vertex https://en.wikipedia.org/wiki/Constructor_(object-oriented_programming)[constructors] are presented below.
...
<4> Coercion to a `vertex` is not possible given.

If a `vertex\<=int` constructor will be used repeatedly, it can either be added to the model or specified in the type prefix. The example below appends a constructor to the type prefix. All subsequent programs will now operate this extended `pg_1` model.

[exec]
----
:[model,pg_1][define,vertex<=int-<('id'->_)]                               //<1>
5 => vertex                                                                //<2>
(5;6) => (vertex;vertex)                                                   //<3>
----
<1> The `pg_1` model-ADT is extended with a `vertex` constructor.
<2> A `vertex` is constructed from an `int`.
<3> A `vertex` pair is constructed from an `int` pair.
...
----
:[model,pg_1]
(5;6) => edge<=(int;int)-<('outV'->-<('id'->.0),'inV'->-<('id'->.1))
----

[cols=1,float=right,width=35,frame=none,grid=none]
|====
a|
[exec,prompt="mm> "]
----
...
The final example extends the `pg_1` model with the `vertex` and `edge` constructors. It also makes salient the topic of the next section: *type paths*.

[exec]
----
:[model,pg_1]                     %%
 [define,vertex<=int-<('id'->_),   %
           edge<=(vertex;vertex)-<('outV'->-<('id'->.0),'inV'->-<('id'->.1))]       //<1>
(5;6) => (vertex;vertex)=>edge                                                      //<2>
(5;6) => edge<=(vertex;vertex)
(5;6) => edge                                                                       //<3>
5 => int-<(vertex;vertex) => edge                                                   //<4>
5-<(vertex;vertex)=>edge
----
<1> The `pg_1` model is extended with a vertex constructor and an edge constructor.
<2> An `int` pair morphed into a `vertex` pair and then into an `edge`.
<3> An `int` pair morphed into an `edge`.
<4> An `int` split into `vertex` clone pairs and then morphed into a https://en.wikipedia.org/wiki/Loop_(graph_theory)[self-loop] edge.
