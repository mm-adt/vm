...
The `[to]` instruction's type definition is `a\<=a[to,_]`. For every incoming \$a \in \tt{obj}\$, there is an outgoing \$a\$ whose path has been extended with `[to]`. The argument to `[to]` is a *named anonymous type*. An example is provided below.
...
Suppose the above anonymous type is evaluated with `int` (i.e., the root of the type is now `int`). This triggers a cascade of events whereby `[plus,1]` maps `int` to `int[plus,1]`, then `[to,x]` maps `int[plus,1]` to `int[plus,1][to,x]`, and so forth. The resultant compiled `int`-type can be evaluated with an `int` value such as 9. The respective commuting diagram below has a top chain that is the *value graph* (evaluation) and two bottom chains that are the *type graph* (compilation). The union of these graphs via the https://en.wikipedia.org/wiki/Inclusion_map[inclusion morphism] (`[type]`) is the `obj` graph of the computation.
...
9 => int => _[plus,1]<x>[plus,2][mult,x]
----
{blank}
|====
...
The key point is that when `[mult,x]` is reached, the `x` is a named anonymous type. This means that the argument to `[mult]` needs to be *resolved* before it can evaluate. To do so, the `obj` graph's edges are searched for a `[to,x]`. When that instruction is found, the range (or domain as it's an identity) replaces `x` and `[mult,10]` is evaluated. The intutition for this process is illustrated on the right.
