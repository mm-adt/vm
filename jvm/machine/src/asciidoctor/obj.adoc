...
A `poly` is a https://en.wikipedia.org/wiki/Free_object[free object]. A free object imposes the equations of an abstract algebra (e.g. \$(a + b) + c = a + (b + c)\$ or \$a \cdot 1 = a\$), but not the equations of the elements of a concrete instance of that algebra (e.g. \$a + b = c\$ or \$a \cdot a = a\$). From a https://en.wikipedia.org/wiki/Data_structure[data structure] perspective, a free object is a https://en.wikipedia.org/wiki/Collection_%28abstract_data_type%29[collection] with semantics strictly tied to the operator(s) of the abstract algebra. That is, free object *collection structures* respect notions of identity, inverse, associativity, and commutativity. An entailment is that a free object `poly` behaves as a https://en.wikipedia.org/wiki/List_(abstract_data_type)[lists] or https://en.wikipedia.org/wiki/Multiset[multisets]. Furthermore, when `poly` elements are types, as opposed to values, common *control structures* are realized such as https://en.wikipedia.org/wiki/Switch_statement[switch] and https://en.wikipedia.org/wiki/Null_coalescing_operator[coalesce]. This bounty of expressiveness is made possible by exploiting mm-ADT type/value-link:https://en.wikipedia.org/wiki/Duality_(mathematics)[duality] at the level of these free objects called `poly`.
...
The carrier set is `obj`, the https://en.wikipedia.org/wiki/Commutative_property[commutative] binary \$\+\$-operator is `,`, and the identity is `{0}`. With `obj` quantification, should two `,-poly` terms have equal _objects_, they can be merged using the mm-ADT `[branch]` `inst` equation:
\[
[\texttt{branch},a_{q_0}, b_{q_1}] =
\begin{cases}
a_{q_0 + q_1}                      & \text{if } a == b, \\
[\texttt{branch},a_{q_0}, b_{q_1}] & \text{otherwise},
\end{cases}
\]
where \$+\$ is the quantifier ring's additive operator. As an abelian group, the operator is commutative which provides set-like semantics as the elements are unordered. However, with quantifier "weighting," `,-lst` collections realize https://en.wikipedia.org/wiki/Multiset[multiset] semantics (also known as bags or weighted sets). For `,-rec` collections, https://en.wikipedia.org/wiki/Multimap[multimap] semantics (https://en.wikipedia.org/wiki/Associative_array[associative arrays] with multiple values for a single key).
