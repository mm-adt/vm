...
When the type is written `int[gt,10]`, the range is deduced.
The domain `int` is applied to `[gt]` to yield `bool`.
The range of `int[gt,10]` is an *anonymous type* (called an *anon* for brevity) and is denoted `_` in `mmlang` (or with no character in most situations).
An anon is a type that has no base ctype -- a wildcard.
A few anonymous *range* examples are provided below.
...
`[plus]` is polymorphic, but `[gt]` isn't.
Thus, the range can be deduced, but without a domain ctype, the internal dtype is unknown.
However, when `int{2}` is provided, it triggers a cascade of deductions that determines all the incoming and outgoing types of each instruction as well as the quantification of those types.
The quantifier ring, when moving "horizontally" across the multiplicative `inst` monoid alters quantifiers using it's multiplication operator, where \$2 \ast 3 \ast 1 = 6\$.
Thus, `bool{6}\<=int{2}[plus,1]{3}[gt,0]`

image::anon-example.svg[width=550,align=center]
