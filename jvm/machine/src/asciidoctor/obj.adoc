...
[.small]
[cols="^1,2,5",width=47,float=left]
|===
| poly | collection | mmlang example

| `,-lst` | https://en.wikipedia.org/wiki/Abelian_group[abelian group] +
https://en.wikipedia.org/wiki/Multiset[multiset]
a|
[exec,prompt="mm> "]
----
...
----
{blank}
| `;-lst` | https://en.wikipedia.org/wiki/Monoid[monoid] +
https://en.wikipedia.org/wiki/List_(abstract_data_type)[list]
a|
[exec,prompt="mm> "]
----
...
----
{blank}
| `\|-lst` | https://en.wikipedia.org/wiki/Module_(mathematics)[module] +
https://en.wikipedia.org/wiki/Scalar_(mathematics)[scalar]
a|
[exec,prompt="mm> "]
----
...
[.small]
[cols="^1,2,5",width=47,float=right]
|===
| poly | collection | mmlang example

| `,-rec` | https://en.wikipedia.org/wiki/Abelian_group[abelian group] +
https://en.wikipedia.org/wiki/Multimap[multimap]
a|
[exec,prompt="mm> "]
----
...
----
{blank}
| `;-rec` | https://en.wikipedia.org/wiki/Monoid[monoid] +
https://en.wikipedia.org/wiki/Associative_array[record]
a|
[exec,prompt="mm> "]
----
...
----
{blank}
| `\|-rec` | https://en.wikipedia.org/wiki/Module_(mathematics)[module] +
https://en.wikipedia.org/wiki/Tuple[pair]
a|
[exec,prompt="mm> "]
----
...
.,-poly Collections
The algebra of the *,-polys* is the additive abelian group of the mm-ADT ring. This means that the `,` (i.e., \$\+\$) is commutative and thus, the terms need not maintain their lexical order. Moreover, given quantification, should two `,-poly` terms be equal at the _object_-level, they can be merged according to the \$+\$-operator's semantics of
\[
a_{q_0} + b_{q_1} =
\begin{cases}
...
\end{cases}
\]
such that when two term objects are not equal, the `poly` is https://en.wikipedia.org/wiki/Irreducible_representation[irreducible]. The lack of order and the "weighting" of quantifiers allows `,-polys` to be used as https://en.wikipedia.org/wiki/Multiset[multisets] (also known as bags or weighted sets). When a `,-poly` is merged via `[merge]` (sugar'd `>-`), those terms that are no longer reducible are the result.

.;-poly Collections
The algebra of the *;-polys* is the non-commutative, multiplicative monoid of the mm-ADT ring. Due to non-commutativity, the terms may not be rearranged around `;` (i.e., \$\ast\$). The benefit of this monoidal algebra is that it enables ordered sequences which effect the semantics of a https://en.wikipedia.org/wiki/List_(abstract_data_type)[list]. Moreover, when a `;-poly` is merged, the \$\ast\$-operator's semantics are
\[
a_{q_0} * b_{q_1} = ab_{q_0 * q_1}.
\]
In other words, the pairwise multiplication common of the https://en.wikipedia.org/wiki/Direct_product[direct product] of two algebraic structures (the _object_ https://en.wikipedia.org/wiki/Semigroup[semigroup] and the _quantifier_ https://en.wikipedia.org/wiki/Ring_%28mathematics%29[ring]). As there is no case-statement in the equation above, every `;-poly` can be reduced with the result of the merge being the final reduced term.
...
The mm-ADT ring's additive abelian (https://en.wikipedia.org/wiki/Commutative_property[commutative]) group operator is accessible via the `[branch]` instruction. The `[branch]` instruction argument is a `poly`, where each term of the `poly` is an operand of the ring's \$+\$-operator. In this way, a `poly` serves as a https://en.wikipedia.org/wiki/Control_flow[control structure]. Due to the prevelant use of this instruction, their is a simple syntactic sugar encoding in `mmlang`: `[ ]`.

[.small]
[cols="^1,2,5",width=47,float=left]
|===
| poly | control | mmlang example
...
The algebra of the *,-polys* is the additive abelian group of the mm-ADT ring. This means that the order in which the terms are evaluated does not effect the result of the computation. More specifically the notion of control, it means that each irreducible term is isolated from the terms others in that they are not sequentially dependent on each other. This isolation of evaluation enables parallelism and realizes the notion of a computational branch.
