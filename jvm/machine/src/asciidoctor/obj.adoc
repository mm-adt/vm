...
===== Poly Types
A `poly` is a https://en.wikipedia.org/wiki/Free_object[free object]. A free object imposes the equations of the abstract algebraic structure it captures (e.g. an https://en.wikipedia.org/wiki/Abelian_group[abelian group] or a https://en.wikipedia.org/wiki/Monoid[monoid]), but not the equations of the elements of a concrete instance of that algebra (e.g. \$a \ast b = c\$ or \$a + a = a\$). From a https://en.wikipedia.org/wiki/Data_structure[data structure] perspective, a free object is a https://en.wikipedia.org/wiki/Collection_%28abstract_data_type%29[collection] with semantics strictly tied to the abstract algebra's operator. That is, free object *collection structures* respect identity, inverse, associativity, and commutativity. The entailment of the free object `poly` is that common collections such as https://en.wikipedia.org/wiki/List_(abstract_data_type)[lists] and https://en.wikipedia.org/wiki/Multiset[multisets] are possible in mm-ADT. Moreover, when the `poly` elements are not values, but types, common *control structures* are realized such as https://en.wikipedia.org/wiki/Switch_statement[switch] and https://en.wikipedia.org/wiki/Null_coalescing_operator[coalesce]. This bounty of expressivity is made possible by exploiting the https://en.wikipedia.org/wiki/Duality_(mathematics)[duality] between types and values at the level of free objects called `poly`.

[.discrete]
====== Poly Structures

There are *value* `polys` and there are *type* `polys`. A value `poly` is composed of only value `objs` and is best understood as a https://en.wikipedia.org/wiki/Collection_%28abstract_data_type%29[collection] data structure. A type `poly` contains at least one type `obj` and is intuitively understood as a https://en.wikipedia.org/wiki/Control_flow[control flow] structure.

As a practical consideration, mm-ADT offers two kinds of `poly`: `lst` (https://en.wikipedia.org/wiki/List_(abstract_data_type)[list]) and `rec` (https://en.wikipedia.org/wiki/Record_%28computer_science%29[record]), where theoretically, `rec` is simply a `lst` with some added conveniences that make common programming use cases easier to express. Finally, every `poly` is associated with one of three algebras: `,` (https://en.wikipedia.org/wiki/Abelian_group[abelian group]), `;` (https://en.wikipedia.org/wiki/Monoid[monoid]), or `|` (https://en.wikipedia.org/wiki/Near-ring[near-ring]). These algebras correspond to the magmas of the mm-ADT https://zenodo.org/record/2565243[stream ring algebra].
...
.,-poly Collections
The *,-polys* capture the additive https://en.wikipedia.org/wiki/Abelian_group[abelian group] of the mm-ADT ring,
\[
(\texttt{obj}, \+, \mathbf{0}),
\]
where the carrier set is `obj`, the https://en.wikipedia.org/wiki/Commutative_property[commutative] binary \$\+\$-operator is `,`, and the identity is `\_{0}`. Being commutative, the terms are unordered. Moreover, given quantification, should two `,-poly` terms have equal _objects_, they can be merged using the mm-ADT \$+\$-operator equation of
\[
a_{q_0} + b_{q_1} =
\begin{cases}
...
\end{cases}
\]
such that when two term objects are not equal, the `poly` is https://en.wikipedia.org/wiki/Irreducible_representation[irreducible]. This is equivalent to the isomorphism between free abelian groups a https://en.wikipedia.org/wiki/Free_module[free module] over \$\mathbb{Z}\$. Given the lack of order along with quantifier "weighting," `,-lst` collections realize https://en.wikipedia.org/wiki/Multiset[multiset] semantics (also known as bags or weighted sets). `,-rec` collections realize https://en.wikipedia.org/wiki/Multimap[multimap] semantics which are https://en.wikipedia.org/wiki/Associative_array[associative arrays] that support multiple values for a single key.

.;-poly Collections
The *;-polys* capture the non-commutative, multiplicative https://en.wikipedia.org/wiki/Monoid[monoid] of the mm-ADT ring
\[
(\texttt{obj},\ast,\mathbf{1}),
\]
where the \$\ast\$-operator is `;` and multiplicative identity is `\_{1}`. Due to non-commutativity, the `;` delimited elements form an https://en.wikipedia.org/wiki/Sequence[ordered sequence]. In `lst`, the consequence is a collection data structure with https://en.wikipedia.org/wiki/List_(abstract_data_type)[list]-semantics. In `rec`, a https://en.wikipedia.org/wiki/Record_%28computer_science%29[record] with ordered fields is realized.

.|-poly Collections
The *|-polys* capture the endomorphic, left-distributive, multiplicative, composition operator over a non-commutative additive group within the mm-ADT https://en.wikipedia.org/wiki/Near-ring[near-ring]
\[
(\texttt{obj},\oplus,\mathbf{0}/\mathbf{1}).
\]
A forthcoming section detailing mm-ADT's ring algebra will discuss the _reduce near-ring_. For now, note that the `|-poly` collections yield singleton `lsts` and `recs`. As such, the `|-polys` are less useful as collections than as controls.

NOTE: When each `poly` kind (`lst` or `rec`) is composed of 0 or 1 term, their behavior's are equivalent. It is only at 2+ terms that their respective algebras are discernible and instructions such as `[eq]` consider the `poly` term separator in their calculation.
