...
Quantifiers are used in three contexts.

* *Type Signature Quantification*

A type signature's _domain_ specifies the type and quantity of `objs` required for evaluation. The _range_ states what can be expected in return. `int{6}\<=int{3}` states that given `3` `ints`, the type will return `6` `insts`. Quantifiers in a type signature are descriptive, used in type checking.

* *Type Description Quantification*

A type description's _instructions_ can be quantified. Their use is deductive, used in type inference.

[exec]
----
int{3}[plus,1]{2}                      //<1>
int{3}[[plus,1],[plus,1]]              //<2>
----
...
When two parallel branches are equivalent, type optimization "collapses" them into a single branch with quantifiers added given the quantifier rings additive (\$+\$) operator. It is more efficient (especially as branches grow in complexity) to evaluate `[plus,1]` once update the branching `objs` using the rings multiplicative (\$\ast\$) operator. The following two examples highlight the fact that type signature quantifiers are used for https://en.wikipedia.org/wiki/Type_system#Static_type_checking[type checking] and type description quantifiers are used for https://en.wikipedia.org/wiki/Type_inference[type inference].
...
[4,5{2}] => int{6}<=int{3}[[plus,1],[plus,1]]
[4,5{2}][[plus,1],[plus,1]]
----
{blank}
<|
...
Type inference deduces the smallest range quantifier given the provided information.
...
int{29} => [is>0]
int{29}[is>0]
{-1{19},3{7},45{3}} => int{29}[is>0]
{-1{19},3{7},45{3}} => int{29}[is>0][count]
----

If a stricter range is provided, then the type inference will only it.
