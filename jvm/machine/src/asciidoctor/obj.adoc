...
Assume the following *anonymous type*.
...
The `[plus]` instruction is https://en.wikipedia.org/wiki/Polymorphism_(computer_science)[polymorphic], but `[gt]` isn't. The anonymous type's range can be deduced. However, without a domain ctype, the internal dtype is also unknown. When `int{2}` is provided, it triggers a cascade of deductions that determines the incoming and outgoing types of each instruction as well as their respective quantifiers (and incident types in the `obj` graph). The quantifier ring, when moving "horizontally" across the multiplicative `inst` monoid alters quantifiers using its multiplication operator, where \$2 \ast 3 \ast 1 = 6\$. Thus, `bool{6}\<=int{2}[plus,1]{3}[gt,0]`
...
_{2}[plus,1]{3}[gt,0]
int{2}[plus,1]{3}[gt,0]
1{2} => int{2}[plus,1]{3}[gt,0]
int{2}[plus,1]{3}[gt,0][explain]
----
{blank}
|
image:anon-example.svg[]
|===
...
The mm-ADT type system can be partitioned into *mono types* (https://en.wikipedia.org/wiki/Monomial[monomials]) and *poly types* (https://en.wikipedia.org/wiki/Polynomial[polynomials]). The `mono` types are https://en.wikipedia.org/wiki/Primitive_data_type[atomic] and there are 4 of them: `bool`, `int`, `real`, and `str`. The associated table presents the typical operators (https://en.wikipedia.org/wiki/Syntactic_sugar[sugared] instructions) that can be applied to each `mono`. The table also includes the additive (*0*) (additive) and multiplicative (*1*) https://en.wikipedia.org/wiki/Identity_element[identity] values of each `mono`.

'''

===== Poly Types

mm-ADT's type system understands types (and their values) as being either https://en.wikipedia.org/wiki/Monomial[monomials] or https://en.wikipedia.org/wiki/Polynomial[polynomials]. Every `obj` is composed of an _object_ (term) and a _quantifier_ (coefficient) -- e.g., `int{3}` or `6{9}`. These _terms_ can then be organized into a composite polynomial structure. The significance of this approach is that mm-ADT itself can be https://en.wikipedia.org/wiki/Embedding[embedded] in an mm-ADT `poly`, thus enabling https://en.wikipedia.org/wiki/Metaprogramming[meta-programming], the unification of products and coproducts as polynomial https://en.wikipedia.org/wiki/Biproduct[biproducts], and the simultaneous realization of both https://en.wikipedia.org/wiki/Data_structure[data] and https://en.wikipedia.org/wiki/Control_flow[control] structures. This section will present the practical aspects of `poly` with a forthcoming section detailing the https://en.wikipedia.org/wiki/Polynomial_ring[polynomial ring] and https://en.wikipedia.org/wiki/Module_(mathematics)[ring module] theory of `poly`.

There are *value*-`polys` and there are *type*-`polys`. A value-`poly` is composed of only value `objs` and is best understood as a classic https://en.wikipedia.org/wiki/Collection_%28abstract_data_type%29[collection] data structure. A type-`poly` is composed of at least one type `obj` and can be understood as denoting a https://en.wikipedia.org/wiki/Stream_processing[streaming] data process. mm-ADT provides two kinds of `poly`: a `lst` (https://en.wikipedia.org/wiki/List_(abstract_data_type)[list]) and a `rec` (https://en.wikipedia.org/wiki/Record_%28computer_science%29[record]).
