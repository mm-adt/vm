...
A model-ADT can be specified using a `rec`, where the `rec` type is the name of the model-ADT. In `mm`, the `'type'` field indexes a https://en.wikipedia.org/wiki/Lookup_table[dictionary] of *type-to-type mappings*. The mm-ADT https://en.wikipedia.org/wiki/Primitive_data_type[base types] are _ground_ in that they are defined in terms of themselves. For example, `bool \-> (bool)` states that `bool` has one definition: `bool\<=bool` or simply `bool`. That is the definition of `bool` is the identity on `bool`. Other model-ADTs require definitions that map from the model's types to the ground types in `mm`.
