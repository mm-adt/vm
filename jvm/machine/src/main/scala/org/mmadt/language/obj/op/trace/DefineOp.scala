...
trait DefineOp {
  ...
  def define(obj: Obj): this.type = DefineOp(UUID.randomUUID().toString, obj).exec(this)
}
object DefineOp extends Func[Obj, Obj] {
  ...
  override def apply(start: Obj, inst: Inst[Obj, Obj]): Obj = if (Obj.fetchOption(start, inst.arg0[StrValue].g).isEmpty) start.via(start, inst) else start
}