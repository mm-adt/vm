...
object PutOp extends Func[Obj, Obj] {
  ...
  override def apply(start: Obj, inst: Inst[Obj, Obj]): Obj = {
    ...
    start match {
      ...
      // REC
      case arec: RecValue[Obj, Obj] => arec.clone(_.replace(key -> value)).via(start, inst)
      case arec: RecType[Obj, Obj] => (oldKey.alive, key.alive, oldValue.alive, value.alive) match {
        ...
        case (true, true, true, false) => arec.clone(_.replace(key -> oldValue)).via(start, Inst.oldInst(inst))
        case _ => arec.clone(_.replace(key -> value)).via(start, inst)
      }
      // LST
      case alst: LstValue[Obj] =>
        val (front, back) = alst.glist.splitAt(key.asInstanceOf[IntValue].g.toInt)
        alst.clone(_ => (front :+ value) ++ back).via(start, inst)
