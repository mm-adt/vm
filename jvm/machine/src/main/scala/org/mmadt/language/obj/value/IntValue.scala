...
trait IntValue extends Int
  ...
  override def one(): this.type = this.clone(name = this.name, value = 1L, q = this.q, via = (this, OneOp()))
  ...
  override def zero(): this.type = this.clone(name = this.name, value = 0L, q = this.q, via = (this, ZeroOp()))
}
