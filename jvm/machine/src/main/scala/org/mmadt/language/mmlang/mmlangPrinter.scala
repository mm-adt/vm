...
object mmlangPrinter {
  ...
  def strmString(strm:Strm[_]):String = if (!strm.alive) zeroObj.toString else typeName(strm) + strm.drain.foldLeft(LBRACKET)((a, b) => a + b.toString + COMMA).dropRight(1) + RBRACKET
