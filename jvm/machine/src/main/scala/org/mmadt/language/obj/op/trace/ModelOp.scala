...
object ModelOp extends Func[Obj, Obj] {
  ...
  val NOMAP: collection.Map[Type[Obj], Lst[Obj]] = Map.empty
  ...
  class RichModel(val model: Model) {
    ...
    final def merging(other: Model): Model = {
      ...
      val x: Model = other.g._2.getOrElse(ModelOp.TYPE, rec(g = (Tokens.`,`, NOMAP))).g._2.flatMap(x => x._2.g._2).foldLeft(model)((a, b) => a.defining(b))
