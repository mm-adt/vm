...
object ModelOp extends Func[Obj, Obj] {
  ...
  private def findTypeBase[A <: Obj](model: Model, label: String): Iterable[A] = model.g._2.getOrElse(TYPE, NOREC).g._2.filter(x => x._1.name == label).flatMap(x => x._2.asInstanceOf[Lst[A]].g._2)
