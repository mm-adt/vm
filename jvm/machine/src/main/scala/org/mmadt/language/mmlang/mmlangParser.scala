...
import org.mmadt.storage.obj.value.{VLst, VRec}
 ...
 */
  ...
  lazy val recType: Parser[RecType[Obj, Obj]] = (LROUND ~> recStruct(obj)) <~ RROUND ^^ (x => new TRec[Obj, Obj](g = x))
  lazy val recCType: Parser[RecType[Obj, Obj]] = Tokens.rec ~> (LROUND ~> recStruct(obj)) <~ RROUND ^^ (x => new TRec[Obj, Obj](g = x)) | (opt(valueType) <~ opt(COLON)) ~ (LROUND ~> recStruct(obj)) <~ RROUND ^^ (x => new TRec[Obj, Obj](g = x._2).named(x._1.getOrElse(Tokens.rec))) | Tokens.rec ^^ (_ => rec)
  lazy val lstType: Parser[LstType[Obj]] = (LROUND ~> lstStruct(obj, RROUND)) <~ RROUND ^^ (x => new TLst[Obj](g = (x._1, x._2)))
  lazy val lstCType: Parser[LstType[Obj]] = Tokens.lst ~> (LROUND ~> lstStruct(obj, RROUND)) <~ RROUND ^^ (x => new TLst(g = x)) | Tokens.lst ^^ (_ => lst)
  lazy val tokenType: Parser[__] = varName ^^ (x => __(x))

  lazy val cType: Parser[Type[Obj]] = (anonType | tobjType | boolType | realType | intType | strType | (not(inst) ~> (lstCType | recCType | lstType | recType)) | tokenType) ~ opt(quantifier) ^^ (x => x._2.map(q => x._1.q(q)).getOrElse(x._1))
  ...
  lazy val lstValue: Parser[VLst[Obj]] = (LROUND ~> lstStruct(objValue, RROUND)) <~ RROUND ^^ (x => new VLst[Obj](g = (x._1, x._2)))
  lazy val recValue: Parser[VRec[Obj, Obj]] =
    (opt(valueType) <~ opt(COLON)) ~ (LROUND ~> recStruct(objValue) <~ RROUND) ^^ (x => new VRec[Obj, Obj](g = (x._2._1, x._2._2.asInstanceOf[Map[Obj, Obj]])).named(x._1.getOrElse(Tokens.rec))) |
      (opt(Tokens.rec) <~ opt(COLON)) ~> (LROUND ~> recStruct(objValue)) <~ RROUND ^^ (x => new VRec[Obj, Obj](g = (x._1, x._2)))
  ...
  lazy val splitSugar: Parser[Inst[Obj, Obj]] = Tokens.split_op ~> (recValue | recType | lstValue | lstType) ^^ (x => SplitOp(x.asInstanceOf[Poly[Obj]]))
  ...
  lazy val splitMergeSugar: Parser[List[Inst[Obj, Obj]]] =
    ((LBRACKET ~> lstStruct(obj, RBRACKET)) <~ RBRACKET) ~ opt(quantifier) ^^ (x => List(SplitOp[Obj](lst(x._1._1, x._1._2.map(x => {
      if (x.isInstanceOf[LstType[_]] && x.root)
        __.is(__.a(x))
      else x
