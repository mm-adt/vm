...
class mmlangParser(val model: Model) extends JavaTokenParsers {
  ...
  lazy val polySep: Parser[String] = Tokens./ | Tokens.| | Tokens.\ | Tokens.`;`
  ...
  lazy val recStruct: Parser[RecTuple[Obj, Obj]] = (LBRACKET ~> repsep((obj <~ (Tokens.:-> | Tokens.::)) ~ obj, COMMA | polySep) <~ RBRACKET) ^^ (x => (Tokens./, x.map(o => (o._1, o._2)).toMap))
  ...
  lazy val recValue: Parser[ORecValue] = opt(valueType) ~ (LBRACKET ~> repsep((objValue <~ (Tokens.:-> | Tokens.::)) ~ objValue, COMMA | polySep) <~ RBRACKET) ^^ (x => vrec(x._1.getOrElse(Tokens.rec), x._2.map(o => (o._1, o._2)).toMap, qOne))
  ...
  lazy val toSugar: Parser[ToInst[Obj]] = LANGLE ~> symbolName <~ RANGLE ^^ (x => ToOp(x))
  lazy val traceSugar: Parser[TraceInst] = LROUND ~> lstObj <~ RROUND ^^ (x => TraceOp(x))
  lazy val fromSugar: Parser[FromInst[Obj]] = LANGLE ~> PERIOD ~ symbolName <~ RANGLE ^^ (x => FromOp(x._2))
