...
class mmlangParser extends JavaTokenParsers {
  ...
  // instruction parsing
  lazy val inst: Parser[Inst[Obj, Obj]] = (liftSugar | sugarlessInst | fromSugar | toSugar | splitSugar | combineSugar | repeatSugar | mergeSugar | infixSugar | getStrSugar | getIntSugar | branchSugar) ~ opt(quantifier) ^^
    (x => x._2.map(q => x._1.q(q)).getOrElse(x._1).asInstanceOf[Inst[Obj, Obj]])
  lazy val infixSugar: Parser[Inst[Obj, Obj]] = not(Tokens.:<=) ~> (
    Tokens.walk_op | Tokens.as_op | Tokens.plus_op | Tokens.mult_op | Tokens.gte_op | Tokens.lte_op | (Tokens.gt_op <~ not(Tokens.gt_op) ^^ { x => x }) |
