...
trait RecType[A <: Obj,B <: Obj] extends Rec[A,B]
  ...
  override def put(key:A,value:B):this.type = this.compose(trec(this.name,this.value() + (key -> value),this.q,this.via.asInstanceOf[ViaTuple[Rec[A,B]]]),PutOp(key,value)).asInstanceOf[this.type]
  override def plus(other:RecType[A,B]):RecType[A,B] = this.compose(trec(name,this.value() ++ other.asInstanceOf[RecType[A,B]].value(),this.q,this.via.asInstanceOf[ViaTuple[Rec[A,B]]]),PlusOp(other))
  override def plus(other:RecValue[_,_]):this.type = this.compose(trec(name,this.value() ++ other.asInstanceOf[RecValue[_,_]].value.asInstanceOf[Map[A,B]],this.q,this.via.asInstanceOf[ViaTuple[Rec[A,B]]]),PlusOp(other)).asInstanceOf[this.type]
