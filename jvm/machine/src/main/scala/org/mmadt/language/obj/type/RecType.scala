...
trait RecType[A <: Obj,B <: Obj] extends Rec[A,B]
  ...
  def value():Map[A,B] //
  def apply(values:(A,B)*):RecType[A,B] = new TRec[A,B](this.name,values.reverse.toMap,this.insts(),this.q()) //
  ...
  override def put(key:A,value:B):RecType[A,B] = new TRec[A,B](this.name,this.value() + (key -> value),this.insts(),this.q()).compose(PutOp(key,value)) //
