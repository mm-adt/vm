...
trait RecStrm[A <: Obj, B <: Obj] extends Rec[A, B]
  ...
  override def plus(other: RecValue[A, B]): RecValue[A, B] = throw new IllegalAccessException() //
  override def is(bool: BoolType): RecType[A, B] = this.start().is(bool) //
  override def is(bool: BoolValue): RecValue[A, B] = throw new IllegalAccessException() //
  ...
  override def get[BT <: Type[_]](key: A, btype: BT): BT = throw new IllegalAccessException() //
}
