...
trait TypeGt[J, V <: Value[V], T <: Type[T]] extends Type[T] {
  ...
  def gt(other: V): BoolType = this.bool(this._jvm() ++ List(inst(Tokens.gt, other)), this.q()) //
  def gt(other: T): BoolType = this.bool(this._jvm() ++ List(inst(Tokens.gt, other)), this.q()) //
