...
trait TypeOr[T <: Type[T]] extends Type[T] {
  ...
  def or(bool: BoolValue): BoolType = this.push(inst(Tokens.or, bool)).asInstanceOf[BoolType] //
  def or(bool: BoolType): BoolType = this.push(inst(Tokens.or, bool)).asInstanceOf[BoolType] //
