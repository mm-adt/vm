...
trait TypeOr[T <: Type[T]] extends Type[T] {
  ...
  def or(bool: BoolValue): BoolType = this.copy(this._jvm() ++ List(inst(Tokens.or, bool)), this.q()).asInstanceOf[BoolType] //
  def or(bool: BoolType): BoolType = this.copy(this._jvm() ++ List(inst(Tokens.or, bool)), this.q()).asInstanceOf[BoolType] //
