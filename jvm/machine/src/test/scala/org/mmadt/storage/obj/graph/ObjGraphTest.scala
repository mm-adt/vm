...
class ObjGraphTest extends FunSuite {
  ...
  test("type construction w/ pg_2") {
    ...
    assertResult('edge(str("outV") -> 'vertex(str("id") -> int(5)) `_,` str("inV") -> 'vertex(str("id") -> int(5))))(graph.model.s(int(5)) -< ('vertex `;` 'vertex) =>>[Obj] 'edge)
  ...
  test("type construction w/ digraph") {
    ...
    assertResult(Seq('attr(str("key") -> str("age") `_,` str("value") -> int(29))))(graph.coerce(str("age") `;` 29, 'attr))
  ...
  test("coercion of base types") {
    ...
    val graph = ObjGraph.create(storage.model('none).defining(str <= int).defining(real <= int).defining(lst))
