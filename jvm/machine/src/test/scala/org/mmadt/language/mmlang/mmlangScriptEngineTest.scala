...
class mmlangScriptEngineTest extends FunSuite {
  ...
  test("test check") {
    ...
    println(engine.eval("[1,2,3] => int{3} =| [plus,8]"))
    //println(engine.eval("[1,2,3,4,5] => int{5} =| [plus,x] => str =[+'1',+'2',+'3']=> int =|[mult,x]"))
  ...
  test("num") {
    ...
    println(engine.eval("(5.0;-1.0) => cmplx"))
  }
  ...
  test("explain instruction parsing") {
    ...
    assert(engine.eval("int[model,mm][define,nat<=int[is>0]]<x>[plus,[mult,x]][[is,[a,nat]][plus,10]|[define,nonat<=int[plus,0]]]<y>[plus,x][explain]").toString.contains("nat->nat<=int[is,bool<=int[gt,0]] x->int nonat->nonat<=int[plus,0]"))
  ...
  test("choice with given") {
    ...
    // TODO: assertResult("'aa'")(engine.eval("'a'-<(int+1[is>0] -> +10 | str -> +'a')>-").toString)
    ...
    // TODO    assertResult(int(10))(engine.eval("10-<(bool{?} -> bool | int -> int)>-"))
    // TODO    assertResult(int(10))(engine.eval("10-<(bool{?} -> true | int -> int)>-"))
...
//    assertResult(str("b".q(6), "c".q(8)))(engine.eval("'a'{2}['b'{3},'c'{4}]"))
  ...
  test("parallel with rec types") {
    ...
    //TODO    assertResult(zeroObj)(engine.eval("10-<(bool{?} -> true ; int -> int[plus,1])>-"))
    //TODO    assertResult(zeroObj)(engine.eval("[10,50,60]-<(bool{?} -> true ; int -> int[plus,1])>-"))
    ...
    /*    assertResult(int(302, 42))(engine.eval(
          """ (0,1,2,3)>-
            | [plus,1][is>2][
            |     int[is>3] -> int[mult,10]
            |   | int[is<4] -> int[mult,100]][plus,2]""".stripMargin))*/
  ...
  test("rec poly") {
    assertResult(int(14))(engine.eval("4-<([is,[a,str{?}]]->'x'|int->+10)>-"))
  ...
  test("poly split/merge/get") {
    ...
    assertResult("(3|int)")(engine.eval("int-<(3|int|int[plus,2])").rangeObj.toString)
    assertResult("int{?}<=int[split,(3|int|int{?}<=int[is,bool<=int[lt,0]])][merge][plus,1]")(engine.eval("int-<(3|int|int[is<0])>-[plus,1]").toString) // TODO: superset
    /////
...
//    assertResult("nat")(engine.eval("int[define,nat<=int[is>0]][as,nat][plus,10]").name)
    ...
    assertResult(btrue `,` bfalse `,` btrue)(engine.eval(
      """ 1
        | [define,nat<=int[is>0],z<=int[is==[zero]],o<=int[is==[one]]]
  ...
  test("defined types") {
    engine.eval(":[model,mm][define,nat<=int[is>0]]")
    assertResult(5.named("nat"))(engine.eval("5 => nat"))
    assertThrows[LanguageException] {
      engine.eval("0 => nat")
    }
    engine.eval(":[model,mm][define,list<=[(_){?}|(_,list)]]")
