...
class pgTest extends BaseInstTest(
  testSet("property graph #1", storage.model('pg_1),
    ...
    IGNORING("eval-[2-5]")(5, int.-<(int `,` int.plus(10)).as('vertex `;` 'vertex), lst(g = (Tokens.`;`, List(('vertex(str("id") -> 'nat(5) `_,` str("label") -> str("vertex"))), ('vertex(str("id") -> 'nat(15) `_,` str("label") -> str("vertex")))))),
