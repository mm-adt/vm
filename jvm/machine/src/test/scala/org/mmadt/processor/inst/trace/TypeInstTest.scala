...
class TypeInstTest extends BaseInstTest(
  testSet("[type] table", List(MM, MMX),
    ...
    comment("[type]"),
    testing(int, int.plus(1).`type`, int.plus(1).`type`, "int[plus,1][type]"),
    testing(int, plus(1).`type`, int.plus(1).`type`, "int[plus,1][type]"),
    testing(__, int.plus(1).`type`, int.plus(1).`type`, "int[plus,1][type]"),
    testing(__, plus(1).`type`, plus(1).`type`, "[plus,1][type]"),
