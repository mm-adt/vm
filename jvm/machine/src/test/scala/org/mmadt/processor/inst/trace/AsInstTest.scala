...
class AsInstTest extends BaseInstTest(
  testSet("[as] w/ values", List(MMX),
    ...
    testing(3, as(str), "3"),
    testing(3, as(str.plus("a")), "3a"),
    testing(int, as(int.plus(1)), int.as(int.plus(1))),
    testing(int, as(real), int.as(real)),
    comment("real"),
    ...
    testing(4.0, as(int), 4),
    testing(4.0, as(int.plus(2)), 6),
    ...
    testing((int(1) `,` 2 `,` 3), as((int.plus(1) `,` int.plus(2) `,` int.plus(3))), (int(2) `,` 4 `,` 6)),
