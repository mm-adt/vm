...
class ZeroInstTest extends FunSuite  with TableDrivenPropertyChecks {
  ...
  test("[zero] testing") {
    forEvery(zeroExpressions){ obj=> {
      ...
      obj match {
        case _:Type[_] => assertResult((obj,ZeroOp().q(2)))(expr.lineage.head)
        case _:Value[_] =>assertResult((obj,ZeroOp()))(expr.lineage.head)
      }
      obj match {
        case _:Type[_] => assertResult((obj.zero().q(2),ZeroOp().q(10)))(expr.lineage.last)
        case _:Value[_] => assertResult((obj.zero().q(2),ZeroOp()))(expr.lineage.last)
      }
    }
    }}
