...
class IsInstTest extends BaseInstTest(
  testSet("[is] table test", List(MM, MMX),
    comment("int"),
    ...
    testing(int, int.is(true), int.is(true), "int[is,true]"),
    ...
    testing(int(1, 2, 3), int.q(3).is(gte(2)).q(10), int(2.q(10), 3.q(10)), "[1,2,3] => int{3}[is,>=2]{10}"),
    ...
    testing(real, real.is(gt(2.0)), real.is(real.gt(2.0)), "real[is>2.0]"),
    IGNORING("eval-[3-5]")(real, is(bool), real.is(bool), "real[is,bool]"),
