...
class ParserTest {
    ...
    private final static LanguageArgs[] PARSING = new LanguageArgs[]{
            ...
            args(ints(11), "11 => int"),
            args(objs(ints(11).bind("a")), "11 => int~a"),
            ...
            args(ints(1), "1 => 1"),
            args(ints(1), "1 <= 1"),
            args(ints(1).bind("a"), "1 => 1~a"),
            args(ints(1).bind("a"), "1 <= 1~a"),
            args(objs(), "1 => 2"),
            args(objs(), "1 <= 2"),
            args(ints(1), "1 => int"),
            args(ints(1).bind("a"), "1 => int~a"),
            args(objs(), "1 => str"),
            args(objs(), "1 => str~a"),
            args(objs(), "1 <= str"),
            args(objs(), "1 <= str~a"),
            ...
            // args(ints(1), "int => (1 <= [id])"),
            args(ints(), "int => int"),
            args(ints().bind("a"), "int => int~a"),
            args(ints().plus(2), "int => (int <= [plus,2])"),
            args(ints().plus(2), "int => (int => [plus,2])"),
            args(ints().<Int>bind("a").plus(2), "int => (int~a => [plus,2])"),
            // args(ints().<Int>label("a").plus(2), "(int => int~a) <= [plus,2]"),
            //args(ints().<Int>label("a").<Int>access(plus(2)), "int => (int~a <= [plus,2])"),
            args(ints().bind("a").plus(2), "int => int~a => [plus,2]"), // TODO: all mutations should drop label
            args(ints().bind("a").<Int>ref(plus(2)), "int <= int~a <= [plus,2]"),
            args(ints().bind("a").<Int>ref(plus(2)), "int <= (int~a <= [plus,2])"),
