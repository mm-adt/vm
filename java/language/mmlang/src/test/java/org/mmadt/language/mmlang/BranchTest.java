...
class BranchTest {
    ...
    private final static LanguageArgs[] BRANCHING = new LanguageArgs[]{
            args(TRec.of(1, "b"),
                    "[:] + (1->'b')"),
            ...
            args(objs(strs("marko").bind("b")),
                    "'marko' => [ int~a | str~b | real~c ] => [is,[a,str]]"),
            args(objs(1, 2, 3),
                    "1 => ([id] + [plus,1] + [plus,2])"),
            args(objs(-1, -2, -3),
                    "1 => (([id] + [plus,1] + [plus,2]) * [neg]) => [plus,[zero]] => int"),
            args(objs(ints(1).bind("a"), ints(2).bind("a"), ints(3).bind("a")),
