 ...
 */
public final class InvMap<S extends WithDiv<S>> extends AbstractFunction implements MapFunction<S, S> {

    private InvMap(final Q quantifier, final String label) {
    ...
    public static <S extends WithDiv<S>> InvMap<S> compile(final Inst inst) {
        return new InvMap<S>(inst.q(), inst.label());
    }
}